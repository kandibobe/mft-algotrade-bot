...
class HealthCheck:
    ...
    async def check_exchange(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if hasattr(self.exchange_client, 'fetch_ticker'):
                if asyncio.iscoroutinefunction(self.exchange_client.fetch_ticker):
                    ticker = await asyncio.wait_for(
                        self.exchange_client.fetch_ticker("BTC/USDT"),
                        timeout=5.0
                    )
                else:
                    # Sync version
            ...
            return {
                "status": "unhealthy",
                "details": "Exchange API timeout (5s)",
                "healthy": False
            }
        except Exception as e:
            ...
            return {
                "status": "unhealthy",
                "details": f"Exchange error: {str(e)}",
                "healthy": False
            }
    ...
    async def check_ml_model(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "status": "unhealthy",
                "details": f"ML model error: {str(e)}",
                "healthy": False
            }
...
if __name__ == "__main__":
    ...
    uvicorn.run(
        "src.monitoring.health_check:app",
        host=args.host,
        port=args.port,
        reload=args.reload
    )
