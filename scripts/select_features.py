...
from src.ml.training.labeling import TripleBarrierLabeler
...
def load_and_prepare_data(pair: str = "BTC/USDT", months: int = 3) -> Tuple[pd.DataFrame, pd.DataFrame]:
    """
    ...
    if len(features_df) < 100:
        raise ValueError(f"Not enough data after cleaning: {len(features_df)} samples. Need at least 100.")
...
def main():
    ...
    parser.add_argument("--pair", type=str, default="BTC/USDT", 
                       help="Trading pair to use for feature selection")
    parser.add_argument("--months", type=int, default=3,
                       help="Number of months of data to use")
    parser.add_argument("--use-shap", action="store_true", default=True,
                       help="Use SHAP values for feature importance (more accurate)")
    parser.add_argument("--no-shap", action="store_false", dest="use_shap",
                       help="Don't use SHAP values, use built-in feature importance")
    parser.add_argument("--top-percentile", type=float, default=0.5,
    ...
    parser.add_argument("--output", type=str, default="user_data/selected_features.json",
    ...
    try:
        ...
        logger.info(f"Reduction: {len(feature_cols) - len(final_features)} features removed "
