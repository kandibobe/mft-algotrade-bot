...
class DataPreprocessor:
    ...
    def _add_log_returns(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.info(f"Added log returns. Mean: {result['log_returns'].mean():.6f}, Std: {result['log_returns'].std():.6f}")
    ...
    def _remove_outliers(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Identify outliers
        outlier_mask = (result['log_returns'].abs() > self.config.outlier_sigma_threshold * returns_std)
        ...
        if combined_mask.any():
            ...
            result.loc[outlier_mask, 'log_returns'] = np.sign(result.loc[outlier_mask, 'log_returns']) * self.config.outlier_sigma_threshold * returns_std[outlier_mask]
...
class AdvancedFeatureEngineer:
    ...
    def __init__(self, config: Optional[AdvancedFeatureEngineerConfig] = None):
    ...
    def _add_microstructure_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        result['volume_position'] = (result['volume'] - volume_low_20) / (volume_high_20 - volume_low_20 + 1e-10)
    ...
    def _add_indicator_lags(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        indicator_cols = [col for col in result.columns if any(
            indicator in col.lower() for indicator in ['rsi', 'macd', 'stoch', 'atr', 'bb', 'adx']
        )]
    ...
    def _normalize_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        exclude_cols = ['open', 'high', 'low', 'close', 'volume', 'hour', 'day_of_week', 
                       'day_of_month', 'month', 'week_of_year', 'is_london_session',
                       'is_us_session', 'is_asian_session', 'is_weekend']
        ...
        if self.config.normalize_method == "rolling":
            ...
            for col in cols_to_normalize:
                rolling_mean = result[col].rolling(window=self.config.rolling_window, min_periods=1).mean()
                rolling_std = result[col].rolling(window=self.config.rolling_window, min_periods=1).std()
...
class TripleBarrierWithPurging:
    ...
    def __init__(self, config: Optional[TripleBarrierWithPurgingConfig] = None):
    ...
    def create_labels(self, df: pd.DataFrame) -> pd.Series:
        ...
        logger.info(f"Buy signals: {label_counts.get(1, 0)} ({label_counts.get(1, 0)/len(labels)*100:.1f}%)")
...
class WalkForwardValidator:
    ...
    def __init__(self, config: Optional[WalkForwardValidatorConfig] = None):
    ...
    def create_folds(self, X: pd.DataFrame, y: pd.Series) -> List[Tuple]:
        ...
        for i, (train_idx, test_idx) in enumerate(folds):
            ...
            logger.info(f"Fold {i+1}: Train {len(train_idx)} samples ({train_dates[0]} to {train_dates[1]}), "
                       f"Test {len(test_idx)} samples ({test_dates[0]} to {test_dates[1]})")
        
        return folds
    
    def cross_validate(self, X: pd.DataFrame, y: pd.Series, model, 
                      metric_func: callable) -> Dict[str, List[float]]:
        """
        ...
        logger.info(f"Walk-Forward CV results: Test score = {avg_test_score:.4f} ± {std_test_score:.4f}")
...
class TradingMetrics:
    ...
    def __init__(self, config: Optional[TradingMetricsConfig] = None):
        self.config = config or TradingMetricsConfig()
    
    def calculate_metrics(self, df: pd.DataFrame, predictions: pd.Series, 
        ...
        risk_metrics = self._calculate_risk_metrics(profit_metrics.get('returns_series', pd.Series()))
    ...
    def _calculate_precision_metrics(self, predictions: pd.Series, 
                                   labels: pd.Series) -> Dict[str, float]:
    ...
    def _calculate_profit_metrics(self, df: pd.DataFrame, predictions: pd.Series,
                                labels: pd.Series) -> Dict[str, float]:
        ...
        for _, trade in trades.iterrows():
            if trade['entry_time'] in returns_series.index and trade['exit_time'] in returns_series.index:
                # Distribute return over trade duration
                trade_duration = (trade['exit_time'] - trade['entry_time']).total_seconds() / (24 * 3600)
                if trade_duration > 0:
                    daily_return = (1 + trade['return_pct']) ** (1 / trade_duration) - 1
                    # Mark trade period
                    trade_mask = (returns_series.index >= trade['entry_time']) & (returns_series.index <= trade['exit_time'])
    ...
    def _simulate_trades(self, df: pd.DataFrame, predictions: pd.Series,
