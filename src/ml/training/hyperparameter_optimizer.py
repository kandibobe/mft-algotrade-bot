...
class HyperparameterOptimizerConfig:
    ...
    timeout: Optional[int] = 3600  # Timeout in seconds (1 hour)
...
class HyperparameterOptimizer:
    ...
    def __init__(self, config: Optional[HyperparameterOptimizerConfig] = None):
    ...
    def _objective_function(self, trial: Trial, X: pd.DataFrame, 
                           y: pd.Series, model_type: str) -> float:
        """
        ...
        if self.config.optimize_for == "precision" and avg_score < self.config.min_precision_threshold:
        ...
        if model_type == "xgboost":
            ...
            params = {
                "n_estimators": trial.suggest_int("n_estimators", 
                                                 *self.config.xgb_n_estimators_range),
                "max_depth": trial.suggest_int("max_depth", 
                                              *self.config.xgb_max_depth_range),
                "learning_rate": trial.suggest_float("learning_rate", 
                                                    *self.config.xgb_learning_rate_range,
                                                    log=True),
                "gamma": trial.suggest_float("gamma", 
                                            *self.config.xgb_gamma_range),
                "min_child_weight": trial.suggest_int("min_child_weight", 
                                                     *self.config.xgb_min_child_weight_range),
                "subsample": trial.suggest_float("subsample", 
                                                *self.config.xgb_subsample_range),
                "colsample_bytree": trial.suggest_float("colsample_bytree", 
                                                       *self.config.xgb_colsample_bytree_range),
                "reg_alpha": trial.suggest_float("reg_alpha", 
                                                *self.config.xgb_reg_alpha_range),
                "reg_lambda": trial.suggest_float("reg_lambda", 
                                                 *self.config.xgb_reg_lambda_range),
                "random_state": 42,
                "n_jobs": -1,
                "verbosity": 0,
            ...
            params = {
                "n_estimators": trial.suggest_int("n_estimators", 
                                                 *self.config.rf_n_estimators_range),
                "max_depth": trial.suggest_int("max_depth", 
                                              *self.config.rf_max_depth_range),
                "min_samples_split": trial.suggest_int("min_samples_split", 
                                                      *self.config.rf_min_samples_split_range),
                "min_samples_leaf": trial.suggest_int("min_samples_leaf", 
                                                     *self.config.rf_min_samples_leaf_range),
                "random_state": 42,
                "n_jobs": -1,
    ...
    def _create_weighted_ensemble(self, ensemble_results: Dict[str, Any], 
        ...
        logger.info(f"Ensemble created with weights: {dict(zip([e[0] for e in estimators], weights))}")
