...
async def portfolio_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    crypto_ids_to_fetch = [
        item["asset_id"] for item in portfolio if item["asset_type"] == constants.ASSET_CRYPTO
    ]
    forex_pairs_to_fetch = [
        item["asset_id"] for item in portfolio if item["asset_type"] == constants.ASSET_FOREX
    ]
    ...
    for item in portfolio:
        ...
        if current_price is None:
            line = get_text(
                constants.MSG_PORTFOLIO_ITEM_NO_PRICE,
                lang_code,
                ticker=ticker,
...
async def p_add_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(
            get_text(constants.MSG_P_ADD_FAIL_INVALID_FORMAT, lang_code)
        )
        return
    ...
    if result_code == user_manager.OPERATION_SUCCESS:
        reply_text = get_text(
            constants.MSG_P_ADD_SUCCESS, lang_code, quantity=quantity, ticker=ticker.upper()
        )
        ...
        reply_text = get_text(
            constants.MSG_ADDWATCH_FAIL_INVALID, lang_code, ticker=html.escape(ticker)
        )
        await update.message.reply_text(reply_text)
    else:
