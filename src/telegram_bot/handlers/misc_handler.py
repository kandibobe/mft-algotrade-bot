...
BaseFetchFunctionType = Callable[[aiohttp.ClientSession], Coroutine[Any, Any, tuple[Any | None, str]]]
...
async def _get_cached_or_fetch(
    ...
    cache_entry = shared_data_cache.setdefault(cache_key, {"data": None, "last_fetch": 0, "args": fetch_args if fetch_args else None})

    cached_data = cache_entry.get("data")

    effective_cache_age = max_cache_age if max_cache_age is not None else SHARED_DATA_FETCH_INTERVAL * 1.5
    ...
    if not force_fetch and not should_fetch_due_to_args_mismatch and cached_data and cache_age < effective_cache_age:
        logger.debug(f"Использование кешированных данных для '{cache_key}' (возраст: {cache_age} сек).")
        return cached_data

    effective_message = update.message or (update.callback_query.message if update.callback_query else None)
    if not effective_message:
        logger.error(f"Не найден effective_message для ответа в _get_cached_or_fetch (cache_key: {cache_key})")
        return cached_data

    loading_text_key = constants.MSG_DATA_STALE_FETCHING if cached_data and not force_fetch and not should_fetch_due_to_args_mismatch else constants.MSG_LOADING

    loading_msg_id_key = f"loading_msg_id_{effective_message.chat_id}_{effective_message.message_id}"
    loading_msg = None
    ...
    if not session or session.closed:
        ...
        if loading_msg: await loading_msg.edit_text(get_text(constants.MSG_ERROR_GENERAL, lang_code))
        elif effective_message: await effective_message.reply_text(get_text(constants.MSG_ERROR_GENERAL, lang_code))
        if loading_msg_id_key in context.chat_data: del context.chat_data[loading_msg_id_key]
        return cached_data
        ...
        if status == data_fetcher.STATUS_OK:
            if fresh_data is not None:
                ...
                if loading_msg:
                    ...
                    except Exception as del_err: logger.debug(f"Не удалось удалить сообщение загрузки: {del_err}")
                if loading_msg_id_key in context.chat_data: del context.chat_data[loading_msg_id_key]
                return fresh_data
            else:
             ...
             if cache_key_base == 'gas': error_msg_key = constants.MSG_GAS_API_KEY_MISSING
             elif cache_key_base == 'cryptonews': error_msg_key = constants.MSG_CRYPTO_NEWS_API_KEY_MISSING
             elif cache_key_base == 'news': error_msg_key = constants.MSG_NEWS_API_KEY_MISSING
             else: error_msg_key = constants.MSG_CONFIG_ERROR
        elif status == data_fetcher.STATUS_INVALID_KEY: error_msg_key = constants.MSG_INVALID_API_KEY
        elif status == data_fetcher.STATUS_RATE_LIMIT: error_msg_key = constants.MSG_RATE_LIMIT_ERROR
        elif status == data_fetcher.STATUS_TIMEOUT: error_msg_key = constants.MSG_TIMEOUT_ERROR
        elif status == data_fetcher.STATUS_NO_DATA: error_msg_key = constants.MSG_NO_DATA_AVAILABLE
        elif status == data_fetcher.STATUS_NOT_FOUND:
            if cache_key_base == 'cryptonews': error_msg_key = constants.MSG_CRYPTO_NEWS_NO_NEWS
            elif cache_key_base == 'tvl': error_msg_key = constants.MSG_TVL_NOT_FOUND
            else: error_msg_key = constants.MSG_ERROR_FETCH

    except Exception as e:
        logger.error(f"Непредвиденное исключение при обновлении данных для '{cache_key}': {e}", exc_info=True)
        error_msg_key = constants.MSG_ERROR_GENERAL
        status = data_fetcher.STATUS_UNKNOWN_ERROR

    logger.warning(f"Не удалось обновить данные для '{cache_key}'. Статус: {status}. Ключ сообщения: {error_msg_key}")
    ...
    if loading_msg:
        try: await loading_msg.edit_text(final_error_text)
        except Exception as edit_err: logger.error(f"Не удалось отредактировать сообщение об ошибке для {cache_key}: {edit_err}")
    elif effective_message:
        try: await effective_message.reply_text(final_error_text)
        except Exception as reply_err: logger.error(f"Не удалось отправить сообщение об ошибке (reply) для {cache_key}: {reply_err}")

    if loading_msg_id_key in context.chat_data: del context.chat_data[loading_msg_id_key]

    is_critical_error = status in [data_fetcher.STATUS_CONFIG_ERROR, data_fetcher.STATUS_INVALID_KEY]
    if cached_data and not is_critical_error and not should_fetch_due_to_args_mismatch:
        logger.debug(f"Возвращаем старые данные для '{cache_key}' из-за ошибки получения свежих ({status}).")
        return cached_data
    else:
        logger.debug(f"Возвращаем None для '{cache_key}' (ошибка: {status} или нет старых данных / новые аргументы).")
        return None
...
async def fear_greed_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if isinstance(fng_data, dict):
        try:
            ...
            reply_text = get_text(constants.MSG_FNG_HEADER, lang_code) + "\n" + get_text(constants.MSG_FNG_DATA, lang_code, value=value, classification=html.escape(classification))
...
async def gas_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    gas_data = await _get_cached_or_fetch("gas", data_fetcher.fetch_eth_gas_price, context, update, lang_code)
    effective_message = update.message or (update.callback_query.message if update.callback_query else None)
    if not effective_message: return
    if isinstance(gas_data, dict):
        try:
            ...
            base_fee_str = f"{base:.2f}" if isinstance(base, float) else str(base) if base is not None else "N/A"
            reply_text = get_text(constants.MSG_GAS_HEADER, lang_code) + "\n" + get_text(constants.MSG_GAS_DATA, lang_code, safe=safe, propose=propose, fast=fast, base_fee=base_fee_str)
            await effective_message.reply_text(reply_text, parse_mode=ParseMode.HTML)
        except Exception as e:
...
async def trending_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    trending_data = await _get_cached_or_fetch("trending", data_fetcher.fetch_coingecko_trending, context, update, lang_code)
    effective_message = update.message or (update.callback_query.message if update.callback_query else None)
    if not effective_message: return
        ...
        try:
            ...
            if not trending_data:
                 ...
                 for i, coin in enumerate(trending_data[:7], 1):
                      ...
                      lines.append(get_text(constants.MSG_TRENDING_ITEM, lang_code, rank=i, name=html.escape(name), symbol=html.escape(symbol), market_cap_rank=rank_str))
            reply_text = "\n".join(lines)
...
async def marketcap_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if args:
        try:
            ...
            if not (1 <= n_to_fetch <= max_n):
                await update.message.reply_text(get_text(constants.MSG_MARKETCAP_INVALID_N, lang_code, default=f"⚠️ Please enter a number between 1 and {max_n}."))
                return
        except ValueError:
            await update.message.reply_text(get_text(constants.MSG_MARKETCAP_INVALID_N, lang_code, default=f"⚠️ Invalid number. Please enter a number between 1 and {max_n}."))
            return
    logger.info(f"Запрос /marketcap (N={n_to_fetch}) от user_id: {user_id}")
    async def _fetch_marketcap_data_adapter(session: aiohttp.ClientSession, limit: int) -> tuple[list[dict[str, Any]] | None, str]:
        ...
        data, status = await data_fetcher._fetch_with_retry(session, url, params, source_name=source_name, base_wait=API_COOLDOWN * 1.2)
        if status != data_fetcher.STATUS_OK: return None, status
        if isinstance(data, list):
            formatted_coins = []
            for coin_data in data:
                if isinstance(coin_data, dict):
                    try:
                        formatted_coins.append({"name": coin_data.get("name"), "symbol": coin_data.get("symbol", "").upper(), "current_price": coin_data.get("current_price"), "price_change_percentage_24h": coin_data.get("price_change_percentage_24h"), "market_cap": coin_data.get("market_cap"), "market_cap_rank": coin_data.get("market_cap_rank")})
                    except Exception as e:
    ...
    marketcap_data = await _get_cached_or_fetch("marketcap", _fetch_marketcap_data_adapter, context, update, lang_code, fetch_args=[n_to_fetch], force_fetch=False)
        ...
        try:
            ...
            if not marketcap_data:
                ...
                for i, coin in enumerate(marketcap_data, 1):
                    ...
                    lines.append(get_text(constants.MSG_MARKETCAP_ITEM, lang_code, rank=rank, name=html.escape(name), symbol=html.escape(symbol), price=price_str, change_24h=change_str, market_cap=market_cap_str))
            reply_text = "\n".join(lines)
            await effective_message.reply_text(reply_text, parse_mode=ParseMode.HTML, disable_web_page_preview=True)
        except Exception as e:
...
async def events_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if args:
        ...
        if arg_lower in ["today", get_text(constants.MSG_TODAY, lang_code, default="today").lower()]:
            period = "today"
            period_key_for_header = constants.MSG_EVENTS_PERIOD_TODAY
        elif arg_lower in ["tomorrow", get_text(constants.MSG_TOMORROW, lang_code, default="tomorrow").lower()]:
            period = "tomorrow"
    ...
    if status == data_fetcher.STATUS_OK and isinstance(events, list):
        ...
        for event in events:
            try:
                ...
                flag = get_text(f"events_country_flag_{event['country'].lower()}", lang_code, default=event['country'])
                impact_icon = get_text(f"events_impact_icon_{event['impact'].lower()}", lang_code, default="")
                event_line = get_text(constants.MSG_EVENTS_ITEM_FORMAT, lang_code, time=time_str, flag=flag, impact=impact_icon, title=html.escape(event['title']))
...
async def ta_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not asset_info or asset_info[0] != constants.ASSET_CRYPTO:
        await update.message.reply_text(get_text(constants.MSG_TA_INVALID_ASSET, lang_code, ticker=html.escape(ticker)))
        return
    ...
    hist_data, status = await data_fetcher.fetch_historical_crypto_data(session, asset_id, days_to_fetch=250)
    ...
    if not ta_results:
        await update.message.reply_text(get_text(constants.MSG_TA_DATA_ERROR, lang_code, ticker=ticker))
        return
    ...
    if rsi_val > 70: rsi_desc_key = constants.MSG_TA_RSI_DESC_OVERBOUGHT
    elif rsi_val < 30: rsi_desc_key = constants.MSG_TA_RSI_DESC_OVERSOLD
    else: rsi_desc_key = constants.MSG_TA_RSI_DESC_NEUTRAL
...
async def funding_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    TRUSTED_EXCHANGES = [
        'Binance', 'Bybit', 'OKX', 'dYdX', 'Bitget', 'Kraken',
        'KuCoin', 'Gate.io', 'MEXC', 'BingX'
    ]
    ...
    if isinstance(funding_data, list):
        try:
            filtered_rates = [
                ...
                if item.get('market') in TRUSTED_EXCHANGES
            ]

            if not filtered_rates:
                logger.warning("После фильтрации по биржам не осталось данных о фандинге.")
                await effective_message.reply_text(get_text(constants.MSG_NO_DATA_AVAILABLE, lang_code))
                return
            ...
            for i, item in enumerate(top_10_high, 1):
                lines.append(get_text(constants.MSG_FUNDING_ITEM, lang_code,
                                      rank=i, symbol=item['symbol'],
                                      market=item['market'], rate=item['rate']))
...
async def tvl_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    tvl_value = await _get_cached_or_fetch(
        "tvl",
        data_fetcher.fetch_tvl,
        context,
        update,
        lang_code,
        fetch_args=[protocol_name]
    )
    ...
    if tvl_value is not None:
        ...
        reply_text = get_text(constants.MSG_TVL_HEADER, lang_code,
...
async def ta_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    rsi_desc = get_text(rsi_desc_key, lang_code)
    lines.append(get_text(constants.MSG_TA_RSI_LINE, lang_code, rsi=f"{rsi_val:.2f}", description=rsi_desc))
