...
def _format_status_for_user(status: str, lang_code: str) -> str:
    ...
    return (
        f"({localized_status})"
        if localized_status
        else f"({status.replace('✅', '').replace('ℹ️', '').replace('❌', '').strip()})"
    )
...
def _format_macro_value(key: str, value: float | None) -> str:
    ...
    try:
        if key == "M2":
            return f"${value / 1000:.2f} трлн"
        elif key in ["CPI", "DXY"]:
            return f"{value:.2f}"
        elif key in ["FFR", "UNRATE"]:
            return f"{value:.2f}%"
        else:
            return f"{value:.2f}"
...
def _format_asset_line(
    ticker: str,
    price_data: tuple[float | None, float | None, str],
    lang_code: str,
    is_watchlist: bool = False,
    asset_type_override: str | None = None,
    ta_results: dict[str, Any] | None = None,
) -> str:
    ...
    if asset_type == constants.ASSET_CRYPTO and ta_results:
        ...
        if "rsi" in ta_results:
            ta_lines.append(
                get_text(constants.MSG_REPORT_TA_RSI, lang_code, rsi_value=ta_results["rsi"])
            )
        if ta_results.get("position_key"):
            position_text = get_text(
                ta_results["position_key"], lang_code, **ta_results.get("position_args", {})
            )
            ta_lines.append(
                get_text(
                    constants.MSG_REPORT_TA_SMA_POSITION, lang_code, position_text=position_text
                )
            )
        base_line += "\n" + "\n".join(ta_lines)
    ...
    keyboard = [
        [
            InlineKeyboardButton(
                get_text(constants.MSG_REPORT_BTN_CRYPTO, lang_code),
                callback_data=constants.CB_REPORT_NAV_CRYPTO,
            ),
            InlineKeyboardButton(
                get_text(constants.MSG_REPORT_BTN_MACRO, lang_code),
                callback_data=constants.CB_REPORT_NAV_MACRO,
            ),
        ],
        [
            InlineKeyboardButton(
                get_text(constants.MSG_REPORT_BTN_ONCHAIN, lang_code),
                callback_data=constants.CB_REPORT_NAV_ONCHAIN,
            ),
            InlineKeyboardButton(
                get_text(constants.MSG_REPORT_BTN_GRAPHS, lang_code),
                callback_data=constants.CB_REPORT_NAV_GRAPHS,
            ),
        ],
    ]
...
def _get_back_to_dashboard_keyboard(lang_code: str) -> InlineKeyboardMarkup:
    keyboard = [
        [
            InlineKeyboardButton(
                f"⬅️ {get_text(constants.MSG_REPORT_BTN_BACK, lang_code)}",
                callback_data=constants.CB_REPORT_NAV_MAIN,
            )
        ]
    ]
    return InlineKeyboardMarkup(keyboard)
...
async def _display_main_dashboard(
    update: Update, context: ContextTypes.DEFAULT_TYPE, message_to_edit=None
):
    ...
    update_time_str = report_data.get(
    ...
    if fng_data and fng_data.get("status") == data_fetcher.STATUS_OK:
        fng_line = get_text(
            constants.MSG_SIGNAL_FNG_INFO,
            lang_code,
            value=fng_data["value"],
            classification=html.escape(fng_data["classification"]),
        )

    header = get_text(
        constants.MSG_REPORT_DASHBOARD_HEADER,
        lang_code,
        period=period_days,
...
async def report_command_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    crypto_in_watchlist_ids = {
        item["asset_id"] for item in watchlist if item["asset_type"] == constants.ASSET_CRYPTO
    }
    forex_in_watchlist_pairs = {
        item["asset_id"] for item in watchlist if item["asset_type"] == constants.ASSET_FOREX
    }
    ...
    tasks = {}
    # Цены
    if crypto_to_fetch_ids:
        tasks["crypto_prices"] = data_fetcher.fetch_current_crypto_data(
            session, list(crypto_to_fetch_ids)
        )
    if forex_to_fetch_pairs:
        tasks["forex_rates"] = data_fetcher.fetch_current_forex_rates(
            session, list(forex_to_fetch_pairs)
        )
    tasks["index_prices"] = data_fetcher.fetch_index_prices(
        session, list(constants.MARKET_INDEX_SYMBOLS.keys())
    )
    # История для ТА
    for asset_id in crypto_to_fetch_ids:
        tasks[f"hist_{asset_id}"] = data_fetcher.fetch_historical_crypto_data(
            session, asset_id, 205
        )
    # Макро
    fred_start_dt = (datetime.now() - timedelta(days=max(period_days * 2, 100))).strftime(
        "%Y-%m-%d"
    )
    for key, series_id in constants.SUPPORTED_MACRO_FOR_REPORT.items():
        tasks[f"macro_{key}"] = data_fetcher.fetch_fred_series(session, series_id, fred_start_dt)
    # On-Chain
    tasks["onchain_netflow"] = data_fetcher.fetch_glassnode_metric(
...
async def report_navigation_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not report_data:
        await query.edit_message_text(
            get_text(constants.MSG_REPORT_DATA_EXPIRED, lang_code), reply_markup=None
        )
        return
    ...
    if action == constants.CB_REPORT_NAV_MAIN:
        ...
        if isinstance(crypto_prices, Exception):
            ...
            for asset_id in report_data["watchlist_crypto_ids"]:
                ...
                ta_results = (
                    analysis.calculate_technical_indicators(hist_data) if hist_data else None
                )
                lines.append(
                    _format_asset_line(
                        ticker,
                        crypto_prices.get(asset_id, (None, None, "N/A")),
                        lang_code,
                        is_watchlist=True,
        ...
        for key, _ in constants.SUPPORTED_MACRO_FOR_REPORT.items():
            macro_data, status = data_map.get(
                f"macro_{key}", (None, data_fetcher.STATUS_UNKNOWN_ERROR)
            )
            ...
            lines.append(
                f" • {key}{date_str}: <code>{html.escape(value_str)}</code> {_format_status_for_user(status, lang_code)} {_get_macro_explanation(key, lang_code)}".strip()
            )
        ...
        for ticker, name in constants.MARKET_INDEX_SYMBOLS.items():
            ...
            price_str = (
                ...
                else get_text(constants.MSG_ERROR_FETCH_SHORT, lang_code)
            )
            lines.append(
                f" • {ticker} ({name}): <code>{html.escape(price_str)}</code> {_format_status_for_user(status, lang_code)}".strip()
            )

    elif action == constants.CB_REPORT_NAV_ONCHAIN:
        lines.append(f"\n<b>{get_text(constants.MSG_REPORT_ONCHAIN_HEADER, lang_code)}</b>")
        netflow_data, status = data_map.get(
            "onchain_netflow", (None, data_fetcher.STATUS_UNKNOWN_ERROR)
        )
        if status == data_fetcher.STATUS_OK and netflow_data:
            ...
            desc_key = (
                constants.MSG_REPORT_ONCHAIN_OUTFLOW
                if netflow_value < 0
                else constants.MSG_REPORT_ONCHAIN_INFLOW
            )
            lines.append(
                get_text(
                    constants.MSG_REPORT_ONCHAIN_NETFLOW,
                    lang_code,
                    value=netflow_btc_str,
                    description=get_text(desc_key, lang_code),
                )
            )
        else:
        ...
        for asset_id in report_data["all_crypto_ids"]:
            ...
            graph_buttons.append(
                InlineKeyboardButton(
                    f"📊 {ticker}", callback_data=f"{constants.CB_ACTION_REPORT_GRAPH}{ticker}"
                )
            )
    ...
    try:
        await query.edit_message_text(
            final_text, reply_markup=reply_markup, parse_mode=ParseMode.HTML
        )
...
async def report_graph_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not report_data:
        await context.bot.send_message(
            user_id, get_text(constants.MSG_REPORT_DATA_EXPIRED, lang_code)
        )
        return
    ...
    try:
        ...
        loading_msg = await context.bot.send_message(
            user_id, get_text(constants.MSG_GRAPH_LOADING, lang_code, symbol=ticker)
        )

        hist_data, status = report_data["data_map"].get(
            f"hist_{asset_id}", (None, data_fetcher.STATUS_UNKNOWN_ERROR)
        )

        if status == data_fetcher.STATUS_OK and hist_data:
            hist_for_graph = hist_data[-(period_days + 5) :]  # Берем с запасом
            graph_bytes = graph_generator.generate_candlestick_graph(
                hist_for_graph, ticker, period_days
            )
            if graph_bytes:
                await context.bot.send_photo(user_id, photo=graph_bytes)
            else:
                await context.bot.send_message(
                    user_id, get_text(constants.MSG_GRAPH_ERROR_GENERAL, lang_code, symbol=ticker)
                )
        else:
            await context.bot.send_message(
                user_id, get_text(constants.MSG_TA_DATA_ERROR, lang_code, ticker=ticker)
            )
