...
async def signal_command_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if not isinstance(btc_hist_data, list) or not isinstance(dxy_hist_data, list):
            raise ValueError(get_text(constants.MSG_ERROR_DATA_FORMAT_DETAIL, lang_code, source="BTC/DXY"))

        btc_val_aligned, dxy_val_aligned, common_dates = analysis.align_data_by_date(btc_hist_data, dxy_hist_data)
        ...
        if final_analysis_days < constants.MIN_POINTS_FOR_SIGNAL:
             raise ValueError(get_text(constants.MSG_SIGNAL_ERROR_POINTS, lang_code, actual_points=final_analysis_days, min_points=constants.MIN_POINTS_FOR_SIGNAL, period=analysis_period))

        cpi_for_analysis = None
        if isinstance(cpi_hist_data_full, list) and len(cpi_hist_data_full) >= 1:
            points_to_take = min(max(3, analysis_period // 30), len(cpi_hist_data_full)) if len(cpi_hist_data_full) >= 2 else 1
        ...
        signal_result = analysis.generate_trading_signal(
            ...
            correlation_btc_dxy_val=analysis.calculate_correlation(btc_val_aligned, dxy_val_aligned),
        ...
        signal_text = "\n".join(parts)
        await effective_message.reply_text(signal_text, parse_mode=ParseMode.HTML, reply_markup=InlineKeyboardMarkup(keyboard))
        ...
        await effective_message.reply_text(get_text(constants.MSG_ERROR_ANALYSIS, lang_code), parse_mode=ParseMode.HTML)
...
async def signal_detail_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if graph_bytes:
        ...
        await query.message.reply_text(get_text(constants.MSG_GRAPH_ERROR_GENERAL, lang_code, symbol=title))
