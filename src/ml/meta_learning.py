...
class MetaLearningEnsemble:
    ...
    def train_meta_model(
        ...
        metrics = {
            ...
            **val_metrics,
            ...
            "meta_model_coef": (
                self.meta_model.coef_.tolist() if hasattr(self.meta_model, "coef_") else []
            ),
        }

        # Save training history
        self.training_history.append({"timestamp": pd.Timestamp.now(), "metrics": metrics})

        logger.info(
            f"Meta-model trained. Training accuracy: {metrics.get('train_accuracy', 0):.3f}"
        )
    ...
    def _calculate_confidence(self, base_preds: np.ndarray) -> np.ndarray:
        ...
        if base_preds.shape[1] > 1:
            ...
            with np.errstate(divide="ignore", invalid="ignore"):
                agreement = np.where(
                    total_votes > 0, np.maximum(buy_votes, sell_votes) / total_votes, 0.5
                )
