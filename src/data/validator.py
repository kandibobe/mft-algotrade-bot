...
def validate_ohlcv(df: pd.DataFrame, strict: bool = False) -> Tuple[bool, List[str]]:
    ...
    if strict:
        ...
        is_valid = not any(any(kw in issue for kw in critical_keywords) for issue in issues)
...
def fill_gaps(df: pd.DataFrame, method: str = "ffill", timeframe: str = "5m") -> pd.DataFrame:
    """
