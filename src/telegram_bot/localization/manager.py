...
def get_text(key: str, lang_code: str = DEFAULT_LANGUAGE, default: str | None = None, **kwargs) -> str:
    ...
    if base_string is None:
        logger.warning(f"Ключ локализации '{key}' не найден ни для '{lang_code}', ни для '{DEFAULT_LANGUAGE}'.")
    ...
    try:
        ...
        logger.error(f"Ключ локализации '{key}' (язык: {lang_code}) ожидает аргумент форматирования: {e}. Строка: '{final_string}' Передано: {kwargs}")
        return f"[{key}: Отсутствует Аргумент '{e}']"
    except Exception as e:
        logger.error(f"Ошибка форматирования строки локализации '{key}' (язык: {lang_code}): {e}. Строка: '{final_string}' Передано: {kwargs}")
