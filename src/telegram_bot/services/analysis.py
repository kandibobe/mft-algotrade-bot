...
def calculate_trend(data: list[float]) -> float:
    ...
    try:
        ...
        logger.debug(
            f"Расчет тренда: slope={slope:.4f}, R^2={r_value**2:.3f} для {len(data)} точек"
        )
...
def align_data_by_date(
    data1: list[tuple[DateType, float]], data2: list[tuple[DateType, float]]
) -> tuple[list[float], list[float], list[date]]:
    if not data1 or not data2:
        return [], [], []
    try:
        dict1 = {
            (item[0].date() if isinstance(item[0], datetime) else item[0]): item[1]
            for item in data1
            if isinstance(item, tuple) and len(item) == 2 and isinstance(item[0], (datetime, date))
        }
        dict2 = {
            (item[0].date() if isinstance(item[0], datetime) else item[0]): item[1]
            for item in data2
...
def get_factor_arrow_score(score_value: int) -> str:
    if score_value > 0:
        return f"🟩 (+{score_value} BUY)"
    if score_value < 0:
        return f"🟥 ({score_value} SELL)"
    return "🟨 (0)"
...
def calculate_technical_indicators(
    ...
    if not historical_prices or len(historical_prices) < min_data_points:
        logger.debug(
            f"Недостаточно данных для ТА: получено {len(historical_prices)}, требуется {min_data_points}"
        )
        return None
    ...
    try:
        ...
        return {"rsi": rsi_value, "position_key": position_key, "position_args": position_args}
...
def calculate_detailed_technical_indicators(
    ...
    if not historical_prices or len(historical_prices) < min_data_points:
        logger.debug(
            f"Недостаточно данных для детального ТА: получено {len(historical_prices)}, требуется {min_data_points}"
        )
        return None
    ...
    try:
        ...
        if any(pd.isna(v) for v in results.values()):
            logger.warning(
                f"Один из детальных индикаторов ТА рассчитан как NaN. Результаты: {results}"
            )
            return None
...
def generate_trading_signal(
    ...
    if btc_trend_val > trend_threshold_abs:
        ...
        btc_factor_score = -1
        details[constants.SIGNAL_FACTOR_BTC_TREND] = {
            "reason_key": constants.SIGNAL_DETAIL_BTC_DOWN
        }
    else:
        details[constants.SIGNAL_FACTOR_BTC_TREND] = {
            "reason_key": constants.SIGNAL_DETAIL_BTC_FLAT
        }
    details[constants.SIGNAL_FACTOR_BTC_TREND]["score_val"] = btc_factor_score
    if btc_factor_score > 0:
        buy_score += btc_factor_score
    else:
        sell_score += abs(btc_factor_score)
    ...
    if cpi_data_full and len(cpi_data_full) >= 2:
        ...
        details[constants.SIGNAL_FACTOR_CPI_TREND] = {
            "reason_key": constants.SIGNAL_DETAIL_CPI_ONE_POINT
        }
    ...
    if cpi_numeric_value is not None:
        details[constants.SIGNAL_FACTOR_CPI_TREND]["numeric_value"] = cpi_numeric_value
    if cpi_factor_score > 0:
        buy_score += cpi_factor_score
    else:
        sell_score += abs(cpi_factor_score)

    # 3. Тренд DXY
    dxy_factor_score = 0
    if dxy_trend_val < -trend_threshold_abs:
        dxy_factor_score = 1
        details[constants.SIGNAL_FACTOR_DXY_TREND] = {
            "reason_key": constants.SIGNAL_DETAIL_DXY_DOWN
        }
    elif dxy_trend_val > trend_threshold_abs:
        dxy_factor_score = -1
        details[constants.SIGNAL_FACTOR_DXY_TREND] = {"reason_key": constants.SIGNAL_DETAIL_DXY_UP}
    else:
        details[constants.SIGNAL_FACTOR_DXY_TREND] = {
            "reason_key": constants.SIGNAL_DETAIL_DXY_FLAT
        }
    details[constants.SIGNAL_FACTOR_DXY_TREND]["score_val"] = dxy_factor_score
    if dxy_factor_score > 0:
        buy_score += dxy_factor_score
    else:
        sell_score += abs(dxy_factor_score)
    ...
    if correlation_btc_dxy_val is not None:
        if correlation_btc_dxy_val < -0.5:
            if dxy_trend_val < -trend_threshold_abs:
                corr_factor_score = 1
                corr_reason_key = constants.SIGNAL_DETAIL_CORR_NEG_DXY_DOWN
            elif dxy_trend_val > trend_threshold_abs:
                corr_factor_score = -1
                corr_reason_key = constants.SIGNAL_DETAIL_CORR_NEG_DXY_UP
    details[constants.SIGNAL_FACTOR_CORR] = {
        "reason_key": corr_reason_key,
        "score_val": corr_factor_score,
        "numeric_value": correlation_btc_dxy_val,
    }
    if corr_factor_score > 0:
        buy_score += corr_factor_score
    else:
        sell_score += abs(corr_factor_score)
    ...
    if fng_value is not None:
        if fng_value <= 25:
            fng_factor_score = 1
            fng_reason_key = constants.SIGNAL_DETAIL_FNG_EXTREME_FEAR
        elif fng_value < 46:
            fng_reason_key = constants.SIGNAL_DETAIL_FNG_FEAR
        elif fng_value <= 54:
            fng_reason_key = constants.SIGNAL_DETAIL_FNG_NEUTRAL
        elif fng_value < 75:
            fng_reason_key = constants.SIGNAL_DETAIL_FNG_GREED
        else:
            fng_factor_score = -1
            fng_reason_key = constants.SIGNAL_DETAIL_FNG_EXTREME_GREED
    details[constants.SIGNAL_FACTOR_FNG] = {
        "reason_key": fng_reason_key,
        "score_val": fng_factor_score,
        "numeric_value": fng_value,
    }
    if fng_factor_score > 0:
        buy_score += fng_factor_score
    else:
        sell_score += abs(fng_factor_score)
    ...
    elif buy_score >= sell_score + strong_signal_threshold:
        signal_key = constants.SIGNAL_STRONG_BUY
    elif sell_score >= buy_score + strong_signal_threshold:
        signal_key = constants.SIGNAL_STRONG_SELL
    elif buy_score > sell_score:
        signal_key = constants.SIGNAL_WEAK_BUY
    elif sell_score > buy_score:
        signal_key = constants.SIGNAL_WEAK_SELL
    else:
        signal_key = constants.SIGNAL_HOLD

    final_signal_text_for_log = get_text(signal_key, "ru")
    logger.info(
        f"Сгенерирован сигнал: {final_signal_text_for_log} (BUY: {buy_score}, SELL: {sell_score})"
    )
