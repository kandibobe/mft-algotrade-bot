...
class WalkForwardBacktest:
    ...
    def __init__(
        ...
        self.feature_engineer = FeatureEngineer(
            FeatureConfig(
                scale_features=True, scaling_method="standard", include_time_features=True
            )
        )
        ...
        self.backtester_config = BacktestConfig(
            initial_capital=10000.0,
            fee_rate=0.001,  # 0.1% fee
            slippage_rate=0.0005,  # 0.05% slippage
            take_profit=self.labeler_config.take_profit,
            stop_loss=self.labeler_config.stop_loss,
            max_holding_bars=self.labeler_config.max_holding_period,
        )
        self.backtester = VectorizedBacktester(self.backtester_config)
    ...
    def create_combinatorial_purged_windows(
        self, df: pd.DataFrame, n_splits: int = 5, embargo_days: int = 7
    ) -> list[tuple[pd.DataFrame, pd.DataFrame]]:
        """
    ...
    def train_and_test_window(
        ...
        try:
            ...
            if len(X_train) < 100 or len(X_test) < 20:
                ...
                return {"window_id": window_id, "success": False, "error": "Insufficient data"}
    ...
    def _calculate_trading_metrics(
        ...
        """
        Calculate trading performance metrics using VectorizedBacktester.
        """
        ...
        signals_series = pd.Series(trade_signals, index=y_true.index)

        # Run event-driven backtest
        results = self.backtester.run(signals_series, test_df)
        ...
        total_pnl = results["total_return"]

        # Approximate Sharpe from equity curve daily returns
        equity_series = results["equity_curve"]
        daily_returns = equity_series.resample("1D").last().pct_change().dropna()
        ...
        avg_trade_return = trades_df["net_pnl"].mean() / self.backtester_config.initial_capital
    ...
    def _generate_visualization(self, results: dict[str, Any]) -> None:
        ...
        ax2 = axes[0, 1]
        window_pnls = [
            r.get("total_pnl", 0) for r in self.window_results if r.get("success", False)
        ]
        if window_pnls:
        ...
        ax4 = axes[1, 1]
        profit_factors = [
            r.get("profit_factor", 0) for r in self.window_results if r.get("success", False)
        ]
        if profit_factors:
            window_ids = [i for i, r in enumerate(self.window_results) if r.get("success", False)]
...
def main():
    ...
    parser.add_argument(
        "--pair", default="BTC/USDT", help="Trading pair (e.g., BTC/USDT, ETH/USDT)"
    )
    parser.add_argument("--timeframe", default="5m", help="Timeframe (e.g., 5m, 15m, 1h, 4h)")
    parser.add_argument(
        "--train-months", type=int, default=3, help="Training window size in months"
    )
    parser.add_argument("--test-months", type=int, default=1, help="Testing window size in months")
    ...
    parser.add_argument(
        "--data-path", default="user_data/data/binance", help="Path to data directory"
    )
