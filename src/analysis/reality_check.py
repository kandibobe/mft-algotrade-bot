...
def run_reality_check(pairs: list[str] | None = None, hours: int = 24):
    ...
    for pair in pairs:
        ...
        if not live_data:
            logger.warning(
                f"Running Backtest Simulation only (No matching live signals found for {pair} in this window)..."
            )
            live_df = pd.DataFrame(
                columns=["timestamp", "symbol", "live_signal", "live_confidence", "live_regime"]
            )
        else:
                ...
                if files:
                    ...
                    if isinstance(loaded_model, dict):
                        ...
                        if os.path.exists(scaler_path):
                            ...
                            logger.warning(
                                "No scaler found for fallback model. Fitting scaler on current data to allow execution (results may be inaccurate)."
                            )
        ...
        try:
            if getattr(engineer, "needs_fitting", False):
                ...
                engineer.validate_features(
                    features, fix_issues=True, raise_on_error=False, drop_low_variance=False
                )
                ...
                if feature_names:
                    for col in feature_names:
                        if col not in features.columns:
                            features[col] = 0.0
                            logger.warning(
                                f"Feature {col} missing after engineering. Filled with 0."
                            )
            else:
