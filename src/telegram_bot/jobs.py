...
async def fetch_shared_data_job(context: ContextTypes.DEFAULT_TYPE):
    ...
    logger.info(
        "Запуск задачи обновления общих данных (F&G, Gas, Trending, News, Global Market)..."
    )
    ...
    try:
        ...
        if gas_status == data_fetcher.STATUS_OK and isinstance(gas_data, dict):
            ...
            gas_status == data_fetcher.STATUS_CONFIG_ERROR
            and shared_data_cache["gas"]["last_fetch"] == 0
        ):
    ...
    try:
        ...
        if trending_status == data_fetcher.STATUS_OK and isinstance(trending_data, list):
            ...
            logger.warning(
                f"Не удалось обновить Trending Coins (статус: {trending_status}). Кеш не изменен."
            )
    except Exception as e:
    ...
    # Crypto News
    try:
        news_data, news_status = await data_fetcher.fetch_crypto_news(
            session, news_filter=CRYPTO_PANIC_FILTER, limit=NEWS_API_PAGE_SIZE
        )
        if news_status == data_fetcher.STATUS_OK and isinstance(news_data, list):
            ...
            news_status == data_fetcher.STATUS_CONFIG_ERROR
            and shared_data_cache["crypto_news"]["last_fetch"] == 0
        ):
            logger.warning(
                "Ключ CryptoPanic API не установлен, кеш крипто-новостей не будет обновляться."
            )
            shared_data_cache["crypto_news"]["data"] = None
        else:
            logger.warning(
                f"Не удалось обновить Crypto News (статус: {news_status}). Кеш не изменен."
            )
    except Exception as e:
    ...
    try:
        ...
        ) = await data_fetcher.fetch_current_global_market_data(session)
        if global_market_status == data_fetcher.STATUS_OK and isinstance(global_market_data, dict):
            ...
            logger.info(
                f"Кеш Global Market Data (BTC.D) обновлен: {global_market_data.get('btc_dominance', 'N/A')}%"
            )
        else:
            logger.warning(
                f"Не удалось обновить Global Market Data (статус: {global_market_status}). Кеш не изменен."
            )
    except Exception as e:
...
async def fetch_onchain_data_job(context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if status == data_fetcher.STATUS_OK and isinstance(netflow_data, list) and netflow_data:
            ...
            status == data_fetcher.STATUS_CONFIG_ERROR
            and shared_data_cache["onchain_netflow"]["last_fetch"] == 0
        ):
            ...
            logger.warning(
                f"Не удалось обновить On-Chain Netflow (статус: {status}). Кеш не изменен."
            )
    except Exception as e:
...
async def check_price_alerts(context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if assets_to_check[constants.ASSET_CRYPTO]:
            crypto_results = await data_fetcher.fetch_current_crypto_data(
                session, list(assets_to_check[constants.ASSET_CRYPTO])
            )
            for cid, (price, _, status) in crypto_results.items():
                if status == data_fetcher.STATUS_OK:
                    current_prices[(constants.ASSET_CRYPTO, cid)] = price
        if assets_to_check[constants.ASSET_FOREX]:
            forex_results = await data_fetcher.fetch_current_forex_rates(
                session, list(assets_to_check[constants.ASSET_FOREX])
            )
            for pair, (rate, status) in forex_results.items():
                if status == data_fetcher.STATUS_OK:
                    current_prices[(constants.ASSET_FOREX, pair)] = rate
    except Exception as e:
    ...
    user_settings_cache = {
        uid: user_manager.get_settings(uid) for uid in {a["user_id"] for a in all_alerts}
    }
    ...
    for alert in all_alerts:
        ...
        triggered = False
        try:
            if condition == ">" and current_price > target_value:
                triggered = True
            elif condition == "<" and current_price < target_value:
                triggered = True
        except TypeError:
            continue
        ...
        if triggered and (current_ts - last_triggered > ALERT_COOLDOWN_SECONDS):
            ...
            message_text = get_text(
                constants.MSG_PRICE_ALERT_TRIGGERED,
                lang_code,
                asset_id=ticker,
            ...
            try:
                ...
                logger.warning(
                    f"Не удалось отправить алерт user {user_id} (ID {alert_id}): Бот заблокирован."
                )
            except Exception as e:
                logger.error(
                    f"Ошибка отправки алерта user {user_id}, alert_id {alert_id}: {e}",
                    exc_info=True,
                )
...
async def send_daily_digest(context: ContextTypes.DEFAULT_TYPE):
    ...
    unique_crypto_ids = {
        ...
        for item in wl
        if item["asset_type"] == constants.ASSET_CRYPTO
    }

    watchlist_prices = {}
    if unique_crypto_ids:
        crypto_results = await data_fetcher.fetch_current_crypto_data(
            session, list(unique_crypto_ids), include_change=True
        )
        watchlist_prices.update(
            {(constants.ASSET_CRYPTO, cid): data for cid, data in crypto_results.items()}
        )
    ...
    for user_id in subscribed_users:
        try:
            ...
            digest_lines = [
                get_text(
                    "TITLE_DIGEST", lang_code, date=datetime.now(timezone.utc).strftime("%Y-%m-%d")
                )
            ]

            if top_crypto_news_list:
                news_line = get_text(
                    "MSG_CRYPTO_NEWS_ITEM",
                    lang_code,
                    url=top_crypto_news_list[0].get("url", "#"),
                    title=html.escape(top_crypto_news_list[0].get("title", "")),
                    domain=html.escape(top_crypto_news_list[0].get("domain", "")),
                )
                digest_lines.append(get_text("HEADER_DIGEST_NEWS", lang_code, news_line=news_line))

            if volatility_data:
                vol_parts = []
                for g in volatility_data.get("gainers", []):
                    vol_parts.append(f"  📈 {g['symbol']}: <b>{g['change']:.2f}%</b>")
                for l in volatility_data.get("losers", []):
                    vol_parts.append(f"  📉 {l['symbol']}: <b>{l['change']:.2f}%</b>")
                if vol_parts:
                    digest_lines.append(
                        get_text(
                            "HEADER_DIGEST_VOLATILITY", lang_code, volatility="\n".join(vol_parts)
                        )
                    )
            ...
            if btc_dominance_info:
                digest_lines.append(
                    get_text(
                        "MSG_DIGEST_DOMINANCE_LINE",
                        lang_code,
                        value=btc_dominance_info.get("btc_dominance", 0),
                    )
                )
