...
async def _fetch_with_retry(
    ...
    for attempt in range(retries + 1):
        ...
        try:
            logger.debug(
                f"[{source_name}] Попытка {attempt + 1}/{retries + 1}: GET {url} (params: {safe_params})"
            )
            async with session.get(
                ...
                if status_code == 200:
                    ...
                    if "application/json" in content_type:
                        try:
                            ...
                            if "alphavantage" in source_name.lower() and isinstance(
                                json_data, dict
                            ):
                                if "Error Message" in json_data:
                                    logger.warning(
                                        f"[{source_name}] Alpha Vantage вернул ошибку в JSON (200 OK): {json_data['Error Message']}"
                                    )
                                    final_status = STATUS_API_ERROR
                                    return None, final_status
                                if (
                                    "Note" in json_data
                                    and "API call frequency" in json_data["Note"]
                                ):
                                    logger.warning(
                                        f"[{source_name}] Alpha Vantage вернул Rate Limit Note в JSON (200 OK): {json_data['Note']}"
                                    )
                            ...
                            logger.error(
                                f"[{source_name}] Ошибка декодирования JSON (ContentTypeError) с {url}. Content-Type: {content_type}. Ошибка: {json_err}"
                            )
                            final_status = STATUS_FORMAT_ERROR
                            return None, final_status
                        except Exception as e:
                            logger.error(
                                f"[{source_name}] Неожиданная ошибка при чтении JSON с {url}: {e}",
                                exc_info=True,
                            )
                            final_status = STATUS_FORMAT_ERROR
                            return None, final_status
                        ...
                        logger.warning(
                            f"[{source_name}] Получен не-JSON ответ (200 OK) с {url}. Content-Type: {content_type}."
                        )
                        final_status = STATUS_FORMAT_ERROR
                        try:
                            text_response = await response.text()
                            logger.debug(f"[{source_name}] Текст ответа: {text_response[:200]}")
                        except Exception:
                            pass
                        return None, final_status

                try:
                    error_body_text_preview = (await response.text())[:250]
                except Exception:
                    error_body_text_preview = "Не удалось прочитать тело ответа."
                ...
                if status_code == 429:
                    ...
                    if retry_after_header:
                        try:
                            wait_time = max(wait_time, float(retry_after_header))
                        except ValueError:
                            logger.warning(
                                f"[{source_name}] Не удалось распарсить Retry-After: {retry_after_header}"
                            )
                    wait_time = min(wait_time, 60.0)
                    logger.warning(
                        f"[{source_name}] API Rate Limit (429) для {url}. Попытка {attempt + 1}. Ожидание {wait_time:.1f} сек... Ответ: {error_body_text_preview}"
                    )
                    last_exception = aiohttp.ClientResponseError(
                        response.request_info,
                        response.history,
                        status=429,
                        message=f"Rate limit exceeded. Body: {error_body_text_preview}",
                    )

                elif status_code == 404:
                    final_status = STATUS_NOT_FOUND
                    logger.warning(
                        f"[{source_name}] Ресурс не найден (404) по адресу {url}. Ответ: {error_body_text_preview}"
                    )
                    last_exception = aiohttp.ClientResponseError(
                        response.request_info,
                        response.history,
                        status=404,
                        message=f"Not Found. Body: {error_body_text_preview}",
                    )
                    return None, final_status
                    ...
                    if (
                        "invalid api key" in error_body_text_preview.lower()
                        or "access restricted" in error_body_text_preview.lower()
                        or "authentication failed" in error_body_text_preview.lower()
                        or "invalid token" in error_body_text_preview.lower()
                        or "api key" in error_body_text_preview.lower()
                    ):
                        final_status = STATUS_INVALID_KEY
                        logger.error(
                            f"[{source_name}] Ошибка доступа ({status_code}): Неверный или ограниченный API ключ для {url}. Ответ: {error_body_text_preview}"
                        )
                    else:
                        logger.error(
                            f"[{source_name}] Ошибка доступа ({status_code}) для {url}. Ответ: {error_body_text_preview}"
                        )
                    last_exception = aiohttp.ClientResponseError(
                        response.request_info,
                        response.history,
                        status=status_code,
                        message=f"Forbidden/Unauthorized. Body: {error_body_text_preview}",
                    )
                    return None, final_status
                    ...
                    logger.error(
                        f"[{source_name}] Неверный запрос (400) к {url}. Параметры: {safe_params}. Ответ API: {error_body_text_preview}"
                    )
                    last_exception = aiohttp.ClientResponseError(
                        response.request_info,
                        response.history,
                        status=400,
                        message=f"Bad Request. Body: {error_body_text_preview}",
                    )
                    return None, final_status
                    ...
                    final_status = STATUS_API_ERROR
                    logger.warning(
                        f"[{source_name}] Ошибка сервера ({status_code}) при запросе к {url}. Попытка {attempt + 1}. Ответ: {error_body_text_preview}"
                    )
                    last_exception = aiohttp.ClientResponseError(
                        response.request_info,
                        response.history,
                        status=status_code,
                        message=f"Server Error. Body: {error_body_text_preview}",
                    )

                else:
                    final_status = STATUS_UNKNOWN_ERROR
                    logger.warning(
                        f"[{source_name}] Неожиданный HTTP статус {status_code} от {url}. Попытка {attempt + 1}. Ответ: {error_body_text_preview}"
                    )
                    last_exception = aiohttp.ClientResponseError(
                        response.request_info,
                        response.history,
                        status=status_code,
                        message=f"Unexpected HTTP status. Body: {error_body_text_preview}",
                    )
                    return None, final_status
                ...
                if attempt < retries:
                    ...
                    logger.error(
                        f"[{source_name}] Закончились ретраи ({retries}) для {url} после ошибки {status_code}."
                    )
                    return None, final_status
            ...
            logger.warning(
                f"[{source_name}] Ошибка сети/соединения на попытке {attempt + 1} для {url}: {net_err.__class__.__name__}"
            )
            ...
            logger.error(
                f"[{source_name}] Непредвиденное исключение при запросе к {url} на попытке {attempt + 1}: {e.__class__.__name__} - {e}",
                exc_info=True,
            )
            final_status = STATUS_UNKNOWN_ERROR
            last_exception = e
    ...
    logger.error(
        f"[{source_name}] Запрос для {url} завершился без явного успеха или ошибки после всех ретраев. Последний статус: {final_status}. Последнее исключение: {last_exception}"
    )
    return None, final_status
...
async def fetch_fred_series(
    ...
    data, status = await _fetch_with_retry(
        session, base_url, params, source_name=f"FRED-{series_id}"
    )
    ...
    try:
        if not isinstance(data, dict):
            raise TypeError("Ответ FRED не является словарем")
        observations = data.get("observations")
        if not isinstance(observations, list):
            raise TypeError("Поле 'observations' в ответе FRED не является списком")
        ...
        for item in observations:
            ...
            if value_str != "." and date_str:
                try:
                    ...
                    logger.warning(
                        f"[FRED] Ошибка парсинга записи ({series_id}): {item}. Ошибка: {parse_err}"
                    )
                    continue

        if not result:
            logger.warning(
                f"[FRED] ({series_id}): Получен пустой список наблюдений после фильтрации."
            )
        ...
        logger.error(
            f"[FRED] Критическая ошибка обработки данных ({series_id}): {e}", exc_info=True
        )
        return None, STATUS_FORMAT_ERROR


async def fetch_current_crypto_data(
    session: aiohttp.ClientSession, crypto_ids: list[str], include_change: bool = False
) -> dict[str, tuple[float | None, float | None, str]]:
    results: dict[str, tuple[float | None, float | None, str]] = dict.fromkeys(
        crypto_ids, (None, None, STATUS_UNKNOWN_ERROR)
    )
    if not crypto_ids:
        return results
    ...
    if include_change:
        params = {
            ...
            "per_page": min(len(crypto_ids) + 5, 250),
            "page": 1,
            "sparkline": "false",
            ...
            for crypto_id in crypto_ids:
                ...
                if coin_data:
                    try:
                        ...
                        logger.warning(
                            f"[{source_name}] Ошибка парсинга данных для {crypto_id}: {parse_err}. Данные: {coin_data}"
                        )
        ...
        if isinstance(data, dict):
            for crypto_id in crypto_ids:
                if (
                    crypto_id in data
                    and isinstance(data[crypto_id], dict)
                    and vs_currency in data[crypto_id]
                ):
                    try:
                        ...
                        logger.warning(
                            f"[{source_name}] Ошибка парсинга цены для {crypto_id}: {parse_err}. Данные: {data[crypto_id]}"
                        )
                        results[crypto_id] = (None, None, STATUS_FORMAT_ERROR)
                else:
...
async def fetch_historical_crypto_data(
    ...
    try:
        ...
        if not isinstance(raw_data_list, list):
            raise TypeError(f"Поле '{data_type}' не является списком")
        if not raw_data_list:
            raise ValueError(f"Список '{data_type}' пуст")

        daily_values: dict[date, float] = {}
        for item in raw_data_list:
            if not isinstance(item, list) or len(item) != 2:
                logger.warning(
                    f"[{source_name}] Некорректный формат элемента в '{data_type}': {item}"
                )
                continue
            try:
        ...
        if historical_data and historical_data[-1][0] >= today_utc:
            logger.debug(
                f"[{source_name}] Удаление потенциально неполных данных за сегодня ({historical_data[-1][0]})."
            )
            historical_data.pop()
        ...
        if not historical_data:
            logger.warning(
                f"[{source_name}] Не осталось данных за {days_to_fetch} дн. после финальной обрезки."
            )
            return None, STATUS_NO_DATA

        logger.debug(
            f"[{source_name}] Обработано {len(historical_data)} точек за период <= {days_to_fetch} дн."
        )
        return historical_data, STATUS_OK

    except Exception as e:
        logger.error(
            f"[{source_name}] Критическая ошибка обработки исторических данных ({data_type}): {e}",
            exc_info=True,
        )
        return None, STATUS_FORMAT_ERROR
...
async def fetch_top_crypto_volatility(
    session: aiohttp.ClientSession, limit: int = 5, vs_currency: str = "usd"
    ...
    try:
        ...
        valid_coins = [
            ...
            and isinstance(c.get("price_change_percentage_24h"), (int, float))
            and c.get("symbol")
        ...
        def format_coin_data(coin_dict: dict) -> dict[str, Any] | None:
            try:
                ...
                logger.warning(
                    f"[{source_name}] Ошибка форматирования данных монеты: {coin_dict}. Ошибка: {format_err}"
                )
                return None

        top_results = {
            "gainers": [
                formatted for c in gainers[:limit] if (formatted := format_coin_data(c)) is not None
            ],
            "losers": [
                formatted for c in losers[:limit] if (formatted := format_coin_data(c)) is not None
            ],
        }

        logger.debug(
            f"[{source_name}] Данные волатильности получены: {len(top_results['gainers'])} G, {len(top_results['losers'])} L"
        )
        return top_results, STATUS_OK

    except Exception as e:
        logger.error(
            f"[{source_name}] Критическая ошибка при обработке волатильности: {e}", exc_info=True
        )
        return None, STATUS_FORMAT_ERROR


async def fetch_current_forex_rates(
    session: aiohttp.ClientSession, pairs: list[str] = constants.FOREX_PAIRS
) -> dict[str, tuple[float | None, str]]:
    results: dict[str, tuple[float | None, str]] = dict.fromkeys(
        pairs, (None, STATUS_UNKNOWN_ERROR)
    )
    if not pairs:
        return results
    ...
    if ALPHA_VANTAGE_API_KEY:
        ...
        for i, pair in enumerate(pairs):
            if len(pair) == 6:
                ...
                av_tasks[pair] = _fetch_with_retry(
                    session, url, params, retries=1, source_name=f"{source_name_av}-{pair}"
                )
            else:
        ...
        if av_tasks:
            ...
            for pair, result_or_exc in av_data_map.items():
                if isinstance(result_or_exc, Exception):
                    logger.error(
                        f"[{source_name_av}] Исключение при запросе {pair}: {result_or_exc}"
                    )
            ...
            if len(successful_pairs) == len(pairs):
                return results
            logger.warning(
                f"[{source_name_av}] Не все курсы получены ({len(successful_pairs)}/{len(pairs)}). Переход к ExchangeRate-API для оставшихся."
            )
    ...
    logger.debug(
        f"[{source_name_er}] Попытка получить курсы для: {', '.join(pairs_for_fallback)} через ExchangeRate-API"
    )
                ...
                if len(pair) == 6:
                    ...
                    if from_curr == base_currency_er:
                        if to_curr in rates_er:
                            ...
                            results[pair] = (None, STATUS_NOT_FOUND)
                        ...
                        if (
                            rate_from_base is not None
                            and rate_to_base is not None
                            and float(rate_from_base) != 0
                        ):
                            results[pair] = (
                                round(float(rate_to_base) / float(rate_from_base), 5),
                                STATUS_OK,
                            )
...
async def fetch_fear_greed_index(
    ...
    if status != STATUS_OK:
        return None, status
    try:
        if not isinstance(data, dict) or data.get("name") != "Fear and Greed Index":
            raise ValueError("Неверный формат или имя ответа")
        fng_data_list = data.get("data", [])
        if not fng_data_list or not isinstance(fng_data_list, list):
            raise ValueError("Отсутствуют данные F&G в ответе")
        fng_data = fng_data_list[0]
        result = {
            "value": int(fng_data["value"]),
            "classification": str(fng_data["value_classification"]),
        ...
        logger.error(
            f"[{source_name}] Ошибка обработки данных F&G: {e}. Ответ: {data}", exc_info=True
        )
        return None, STATUS_FORMAT_ERROR


async def fetch_eth_gas_price(
    session: aiohttp.ClientSession,
    ...
    data, status = await _fetch_with_retry(
        session, url, params, retries=1, base_wait=1.5, source_name=source_name
    )

    if status != STATUS_OK:
        if (
            isinstance(data, dict)
            and data.get("message") == "NOTOK"
            and "invalid api key" in str(data.get("result", "")).lower()
        ):
            logger.error(
                f"[{source_name}] Etherscan API Key невалиден. Ответ: {data.get('result', '')}"
            )
    ...
    try:
        if (
            ...
            logger.error(
                f"[{source_name}] Неверный статус или формат ответа Etherscan: {data.get('message', '')}. Result: {data.get('result')}"
            )
            if (
                isinstance(data.get("result"), str)
                and "invalid api key" in data.get("result", "").lower()
            ):
                return None, STATUS_INVALID_KEY
            return None, STATUS_FORMAT_ERROR
        ...
        result = {
            "safe": int(float(gas_data["SafeGasPrice"])),
            "propose": int(float(gas_data["ProposeGasPrice"])),
            "fast": int(float(gas_data["FastGasPrice"])),
            "base_fee": round(float(gas_data["suggestBaseFee"]), 2),
        }
        logger.debug(f"[{source_name}] ETH Gas получен: {result}")
        return result, STATUS_OK
    except (ValueError, KeyError, TypeError) as e:
        logger.error(
            f"[{source_name}] Ошибка обработки данных ETH Gas: {e}. Ответ: {data}", exc_info=True
        )
        return None, STATUS_FORMAT_ERROR
...
async def fetch_coingecko_trending(
    ...
    try:
        ...
        if not trending_coins and data["coins"]:
            return None, STATUS_FORMAT_ERROR
        ...
        logger.error(
            f"[{source_name}] Критическая ошибка обработки Trending: {e}. Ответ: {data}",
            exc_info=True,
        )
        return None, STATUS_FORMAT_ERROR
...
async def fetch_crypto_news(
    session: aiohttp.ClientSession, news_filter: str = "important", limit: int = 5
) -> tuple[list[dict[str, Any]] | None, str]:
    ...
    if status != STATUS_OK:
        if status == STATUS_INVALID_KEY:
            ...
            logger.warning(
                f"[{source_name}] CryptoPanic API вернул 404 (Not Found) для фильтра '{news_filter}'."
            )
        return None, status
    ...
    try:
        if not isinstance(data, dict) or not isinstance(data.get("results"), list):
            logger.error(
                f"[{source_name}] Неверный формат ответа от CryptoPanic: 'results' не список или ответ не dict. Ответ: {str(data)[:200]}"
            )
            return None, STATUS_FORMAT_ERROR
        ...
        for item in data["results"][:limit]:
            ...
            if isinstance(item, dict) and item.get("title") and item.get("url"):
                news_items.append(
                    {
                        ...
                        "domain": item.get(
                            "domain", "N/A"
                        ),  # .get() безопасен, вернет 'N/A' если ключа нет
                        "published_at": item.get("published_at"),
                        "source_title": item.get("source", {}).get(
                            "title"
                ...
                if isinstance(item, dict):
                    logger.warning(
                        f"[{source_name}] Пропуск элемента новости из-за отсутствия 'title' или 'url': {item.get('id', 'No ID')}"
                    )
                else:
                    logger.warning(
                        f"[{source_name}] Пропуск невалидного элемента новости (не dict): {str(item)[:100]}"
                    )
            # --- КОНЕЦ ИСПРАВЛЕНИЯ ---

        if not news_items and data["results"]:
            logger.warning(
                f"[{source_name}] Не удалось отформатировать ни одной новости из полученных {len(data['results'])}."
            )
        ...
        logger.error(
            f"[{source_name}] Критическая ошибка обработки новостей CryptoPanic: {e}. Ответ: {str(data)[:200]}",
            exc_info=True,
        )
        return None, STATUS_FORMAT_ERROR
...
def fetch_general_news(
    query: str, language: str = "en", page_size: int = 5, sort_by: str = "relevancy"
) -> tuple[list[dict[str, Any]] | None, str]:
    if not newsapi_client:
        return None, STATUS_CONFIG_ERROR
    if not query:
        return None, STATUS_BAD_REQUEST
    ...
    try:
        ...
        if all_articles.get("status") == "ok":
            ...
            if not formatted_articles and articles_list:
                return None, STATUS_FORMAT_ERROR
            ...
            if err_code in ["apiKeyInvalid", "apiKeyMissing", "apiKeyDisabled"]:
                return None, STATUS_INVALID_KEY
            if err_code == "rateLimited":
                return None, STATUS_RATE_LIMIT
...
async def fetch_index_prices(
    ...
    results: dict[str, tuple[float | None, str]] = dict.fromkeys(
        symbols, (None, STATUS_UNKNOWN_ERROR)
    )
    if not symbols:
        return results
    if not ALPHA_VANTAGE_API_KEY:
        logger.warning(
            "[AlphaVantage-Index] Запрос не выполнен: ключ ALPHA_VANTAGE_API_KEY отсутствует."
        )
    ...
    for i, symbol in enumerate(symbols):
        ...
        data, status = await _fetch_with_retry(
            ...
            params,
            retries=1,
            source_name=f"{source_name}-{symbol}",
        )
        ...
        if status == STATUS_OK and isinstance(data, dict) and "Global Quote" in data:
            try:
                ...
                if not quote_data:
                    logger.warning(
                        f"[{source_name}] Пустой 'Global Quote' для {symbol}. Вероятно, тикер не найден."
                    )
                ...
                if price_str is None:
                    logger.warning(
                        f"[{source_name}] '05. price' отсутствует для {symbol}. Ответ: {data}"
                    )
        ...
        elif status == STATUS_RATE_LIMIT or (
            isinstance(data, dict) and data.get("Note") and "API call frequency" in data["Note"]
        ):
            logger.warning(
                f"[{source_name}] Достигнут Rate Limit для {symbol} (или всех последующих)."
            )
            ...
            if isinstance(data, dict) and "Error Message" in data:
                logger.warning(
                    f"[{source_name}] Ошибка от AlphaVantage для {symbol}: {data['Error Message']}"
                )
...
async def fetch_current_global_market_data(
    session: aiohttp.ClientSession,
) -> tuple[dict[str, Any] | None, str]:
    ...
    try:
        ...
        logger.debug(
            f"[{source_name}] Global market data fetched: BTC.D {result.get('btc_dominance')}%"
        )
        return result, STATUS_OK

    except (ValueError, KeyError, TypeError) as e:
        logger.error(
            f"[{source_name}] Error processing global market data: {e}. Response: {data}",
            exc_info=True,
        )
        return None, STATUS_FORMAT_ERROR
...
async def fetch_historical_btc_dominance_data(
    session: aiohttp.ClientSession, days_to_fetch: int, vs_currency: str = "usd"
) -> tuple[list[tuple[date, float]] | None, str]:
    logger.warning(
        "[Derived-BTCDominanceHist] Historical BTC Dominance data fetching is currently disabled due to API limitations for free tiers."
    )
    return None, STATUS_NO_DATA
    ...
    try:
        ...
        for item in data:
            if (
                ...
                and item.get("contract_type") == "perpetual"
                and isinstance(item.get("funding_rate"), (int, float))
            ):
        ...
        if not funding_rates:
            logger.warning(
                f"[{source_name}] No perpetual contracts with funding rates found in the response."
            )
            return None, STATUS_NO_DATA

        logger.debug(
            f"[{source_name}] Successfully processed {len(funding_rates)} perpetual contracts."
        )
        return funding_rates, STATUS_OK

    except (TypeError, KeyError, ValueError) as e:
        logger.error(
            f"[{source_name}] Critical error processing funding rates data: {e}", exc_info=True
        )
        return None, STATUS_FORMAT_ERROR


async def fetch_tvl(session: aiohttp.ClientSession, protocol_name: str) -> tuple[float | None, str]:
    ...
    try:
        ...
        async with session.get(url, timeout=DEFAULT_REQUEST_TIMEOUT) as response:
            ...
            if status_code == 200:
                ...
                try:
                    ...
                    logger.error(
                        f"[{source_name}] Не удалось преобразовать ответ в число: {tvl_value_str[:100]}"
                    )
                    return None, STATUS_FORMAT_ERROR
            elif status_code == 404:
        ...
        logger.error(
            f"[{source_name}] Непредвиденное исключение при запросе TVL: {e}", exc_info=True
        )
        return None, STATUS_UNKNOWN_ERROR


async def fetch_economic_events(
    session: aiohttp.ClientSession, period: str = "week"
) -> tuple[list[dict[str, Any]] | None, str]:
    """
    ...
    all_simulated_events = [
        {
            "time": now.replace(hour=12, minute=30, second=0, microsecond=0),
            "country": "US",
        ...
        {
            "time": now.replace(hour=14, minute=0, second=0, microsecond=0),
            "country": "US",
        ...
        },
        {
            ...
            "country": "EU",
            "title": "German Flash PMI",
            "impact": "medium",
            "forecast": "46.5",
            "previous": "45.7",
        },
        {
            "time": (now + timedelta(days=1)).replace(hour=13, minute=45, second=0, microsecond=0),
            "country": "US",
        ...
        {
            "time": (now + timedelta(days=3)).replace(hour=8, minute=30, second=0, microsecond=0),
            "country": "EU",
        ...
        {
            "time": (now + timedelta(days=4)).replace(hour=12, minute=30, second=0, microsecond=0),
            "country": "US",
        ...
        {
            "time": (now + timedelta(days=4)).replace(hour=14, minute=0, second=0, microsecond=0),
            "country": "US",
        ...
        {
            "time": (now + timedelta(days=8)).replace(hour=14, minute=0, second=0, microsecond=0),
            "country": "US",
    ...
    if period == "today":
        ...
        filtered_events = [
            e for e in all_simulated_events if today <= e["time"].date() < end_of_week
        ]
...
async def fetch_glassnode_metric(
    ...
    data, status = await _fetch_with_retry(
        session, url, params=params, headers=headers, source_name=source_name
    )
    ...
    try:
        ...
        formatted_data = [
            {"t": item["t"], "v": item["v"]} for item in data if "t" in item and "v" in item
        ]
        ...
        logger.error(
            f"[{source_name}] Критическая ошибка обработки данных Glassnode: {e}", exc_info=True
        )
        return None, STATUS_FORMAT_ERROR
