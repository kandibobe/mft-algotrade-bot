...
async def news_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not query_list:
        await effective_message.reply_text(get_text(constants.MSG_NEWS_PROMPT, lang_code), parse_mode=ParseMode.HTML)
        return
    ...
    news_articles, status = result_tuple if isinstance(result_tuple, tuple) else (None, data_fetcher.STATUS_UNKNOWN_ERROR)
    ...
    if status == data_fetcher.STATUS_OK and isinstance(news_articles, list) and news_articles:
        ...
        for article in news_articles:
            if isinstance(article, dict):
                lines.append(get_text(
                    constants.MSG_NEWS_ITEM_WITH_SOURCE, lang_code,
                    url=article.get('url', '#'),
    ...
    elif status == data_fetcher.STATUS_NO_DATA or (status == data_fetcher.STATUS_OK and not news_articles):
        await effective_message.reply_text(get_text(constants.MSG_NEWS_NO_RESULTS, lang_code, query=html.escape(query)), parse_mode=ParseMode.HTML)
        ...
        error_msg_key = constants.MSG_RATE_LIMIT_ERROR if status == data_fetcher.STATUS_RATE_LIMIT else constants.MSG_TIMEOUT_ERROR if status == data_fetcher.STATUS_TIMEOUT else constants.MSG_ERROR_FETCH
