...
class MLTrainingPipeline:
    ...
    def load_data(
        self, pairs: list[str], timeframe: str = "5m", days: int = None
        ...
        if self.use_feature_store:
            ...
            try:
                ...
                start_date = (
                    end_date - timedelta(days=days) if days else datetime(2020, 1, 1)
                )  # Default start date if days not specified
        ...
        for pair in pairs:
            ...
            if filetype == "feather":
                ...
                df = pd.DataFrame(
                    json_data, columns=["timestamp", "open", "high", "low", "close", "volume"]
                )
                df["timestamp"] = pd.to_datetime(df["timestamp"], unit="ms")
                df.set_index("timestamp", inplace=True)
            ...
            if self.quick_mode:
                df = df.tail(1000)
                logger.info("  ⚡ Quick mode: Using last 1000 candles")
            elif days:
