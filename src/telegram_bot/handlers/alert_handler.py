    ...
    if not alerts:
        ...
        for alert in alerts:
            ...
            if alert_type == constants.ALERT_TYPE_RSI:
                ...
                alert_text_display = get_text(constants.MSG_ALERT_ITEM_PRICE, lang_code, alert_id=alert['id'], asset_id=ticker, condition=html.escape(alert['condition']), value=alert['target_value'], created_at=created_at_str)

            row = [
                InlineKeyboardButton(f"{get_text(constants.BTN_EDIT, lang_code)} (ID: {alert['id']})", callback_data=f"{CB_EA_START}{alert['id']}"),
                InlineKeyboardButton(get_text(constants.BTN_DELETE, lang_code), callback_data=f"{constants.CB_ACTION_DEL_ALERT}{alert['id']}")
            ]
    ...
    if effective_message:
        if update.callback_query:
            try:
                ...
                if "message is not modified" not in str(e).lower(): logger.warning(f"Не удалось отредактировать список алертов: {e}.")
...
async def delalert_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not alert_data:
        await query.answer(get_text(constants.ERROR_DELALERT_NOTFOUND, lang_code, alert_id=alert_id), show_alert=True)
        await alerts_command(update, context)
        return

    ticker = constants.REVERSE_ASSET_MAP.get(alert_data['asset_id'], alert_data['asset_id'])
    confirmation_text = get_text(constants.MSG_DELALERT_CONFIRM, lang_code, alert_id=alert_id, ticker=ticker, condition=html.escape(alert_data['condition']), value=alert_data['target_value'])
    keyboard = [[
        InlineKeyboardButton(get_text(constants.BTN_YES, lang_code), callback_data=f"{constants.CB_ACTION_DEL_ALERT_CONFIRMED}{alert_id}"),
        InlineKeyboardButton(get_text(constants.BTN_NO, lang_code), callback_data=f"{constants.CB_ACTION_DEL_ALERT_CANCELLED}{alert_id}")
    ]]
    await query.edit_message_text(confirmation_text, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode=ParseMode.HTML)
...
async def delalert_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    result_code = user_manager.delete_user_price_alert(user_id, alert_id)
    if result_code == user_manager.OPERATION_SUCCESS:
        await update.message.reply_text(get_text(constants.MSG_DELALERT_SUCCESS, lang_code, alert_id=alert_id))
    else:
        await update.message.reply_text(get_text(constants.ERROR_DELALERT_NOTFOUND, lang_code, alert_id=alert_id))
    await alerts_command(update, context)
...
async def _handle_quick_price_alert(update: Update, context: ContextTypes.DEFAULT_TYPE, text: str) -> bool:
    ...
    if match_abs:
        ...
        if not asset_info:
            await update.message.reply_text(get_text(constants.ERROR_ADDALERT_INVALID_ASSET, lang_code, ticker=html.escape(ticker)))
            return True
        try:
            target_value = float(value_str.replace(',', '.'))
            if target_value <= 0:
                await update.message.reply_text(get_text(constants.ERROR_VALUE_MUST_BE_POSITIVE, lang_code))
                return True
        except ValueError:
    ...
    if match_pct:
        ...
        try:
            ...
            if percent_change <= 0:
                await update.message.reply_text(get_text(constants.ERROR_ADDALERT_PERCENT_VALUE, lang_code))
                return True
        except ValueError:
            await update.message.reply_text(get_text(constants.ERROR_ADDALERT_PERCENT_VALUE, lang_code))
            return True
        ...
        if asset_type == constants.ASSET_CRYPTO:
            ...
            if rate_tuple: price_data = (rate_tuple[0], None, rate_tuple[1])
...
async def received_alert_type(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if chosen_type == constants.ALERT_TYPE_PRICE: return await ask_price_condition(update, context)
    if chosen_type == constants.ALERT_TYPE_RSI: return await ask_rsi_condition(update, context)
    return ConversationHandler.END
...
async def ask_price_condition(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if session and not session.closed:
        ...
        try:
            ...
            if price_data and price_data[2] == data_fetcher.STATUS_OK and price_data[0] is not None:
                ...
                current_price_text = get_text(constants.MSG_ADDALERT_CURRENT_PRICE, lang_code, price=price_format.format(price_data[0]))
        except Exception as e:
            logger.warning(f"Ошибка получения текущей цены для '{ticker}' в /addalert: {e}", exc_info=True)
    ...
    keyboard = [[
        InlineKeyboardButton(get_text(constants.BTN_CONDITION_GT, lang_code), callback_data=constants.CB_ALERT_COND_GT),
        InlineKeyboardButton(get_text(constants.BTN_CONDITION_LT, lang_code), callback_data=constants.CB_ALERT_COND_LT)
    ]]
    await context.bot.send_message(chat_id=user_id, text=final_prompt_text, reply_markup=InlineKeyboardMarkup(keyboard))
    return ASK_PRICE_CONDITION

async def received_price_condition_button(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    query = update.callback_query
    await query.answer()
    chosen_condition = ">" if query.data == constants.CB_ALERT_COND_GT else "<"
    context.user_data['alert_condition'] = chosen_condition
    ticker = context.user_data.get('alert_ticker', get_text(constants.TEXT_ASSET_DEFAULT, await get_user_language(update.effective_user.id)))
    await query.edit_message_reply_markup(reply_markup=None)
    prompt_text = get_text(constants.PROMPT_ADDALERT_VALUE, await get_user_language(update.effective_user.id), asset_id=html.escape(ticker), condition=html.escape(chosen_condition))
...
async def add_price_alert_finish(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    result_code, new_alert_id = user_manager.add_user_alert(user_id, data['alert_asset_type'], data['alert_asset_id'], data['alert_type'], data['alert_condition'], target_value)

    if result_code == user_manager.OPERATION_SUCCESS:
        reply_text = get_text(constants.MSG_ADDALERT_SUCCESS, lang_code, alert_id=new_alert_id, asset_id=html.escape(data['alert_ticker']), condition=html.escape(data['alert_condition']), value=target_value)
        await update.message.reply_text(reply_text, reply_markup=common_handlers.get_main_keyboard(lang_code))
    else:
        await update.message.reply_text(get_text(constants.ERROR_ADDALERT_GENERIC, lang_code), reply_markup=common_handlers.get_main_keyboard(lang_code))
...
async def ask_rsi_condition(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    lang_code = await get_user_language(user_id)
    keyboard = [[
        InlineKeyboardButton(get_text(constants.BTN_RSI_COND_GT70, lang_code), callback_data=f"{constants.CB_ACTION_SET_RSI_COND}gt70"),
        InlineKeyboardButton(get_text(constants.BTN_RSI_COND_LT30, lang_code), callback_data=f"{constants.CB_ACTION_SET_RSI_COND}lt30")
    ]]
    await context.bot.send_message(chat_id=user_id, text=get_text(constants.PROMPT_ADDALERT_RSI_CONDITION, lang_code), reply_markup=InlineKeyboardMarkup(keyboard))
...
async def received_rsi_condition_button(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if result_code == user_manager.OPERATION_SUCCESS:
        ...
        await context.bot.send_message(chat_id=user_id, text=get_text(constants.ERROR_ADDALERT_GENERIC, lang_code), reply_markup=common_handlers.get_main_keyboard(lang_code))
...
async def cancel_addalert(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    context.user_data.clear()
    await update.message.reply_text(get_text(constants.MSG_ADDALERT_CANCEL, lang_code), reply_markup=common_handlers.get_main_keyboard(lang_code))
    return ConversationHandler.END
...
async def edit_alert_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if alert_data.get('alert_type', constants.ALERT_TYPE_PRICE) != constants.ALERT_TYPE_PRICE:
        await context.bot.send_message(chat_id=user_id, text="Редактирование этого типа алертов пока не поддерживается.")
        return ConversationHandler.END

    context.user_data['edit_alert_id'] = alert_id
    context.user_data['edit_alert_data'] = alert_data
    ticker = constants.REVERSE_ASSET_MAP.get(alert_data['asset_id'], alert_data['asset_id'])
    text = get_text(constants.PROMPT_EDIT_ALERT_CHOICE, lang_code, ticker=ticker, condition=html.escape(alert_data['condition']), value=alert_data['target_value'], alert_id=alert_id)
    keyboard = [
        [InlineKeyboardButton(get_text(constants.BTN_EDIT_CONDITION, lang_code), callback_data=f"{CB_EA_SET_COND_PREFIX}{alert_id}")],
        [InlineKeyboardButton(get_text(constants.BTN_EDIT_VALUE, lang_code), callback_data=f"{CB_EA_SET_VAL}{alert_id}")],
        [InlineKeyboardButton(get_text(constants.BTN_CANCEL, lang_code), callback_data=f"{CB_EA_CANCEL_SINGLE}{alert_id}")]
    ]
...
async def edit_alert_ask_condition(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    ticker = constants.REVERSE_ASSET_MAP.get(alert_data['asset_id'], alert_data['asset_id'])
    text = get_text(constants.PROMPT_EDIT_ALERT_NEW_CONDITION, lang_code, ticker=ticker, current_condition=html.escape(alert_data['condition']))
    keyboard = [
        [
            InlineKeyboardButton(get_text(constants.BTN_CONDITION_GT, lang_code), callback_data=f"{CB_EA_SET_COND_PREFIX}{alert_data['id']}_>"),
            InlineKeyboardButton(get_text(constants.BTN_CONDITION_LT, lang_code), callback_data=f"{CB_EA_SET_COND_PREFIX}{alert_data['id']}_<")
        ],
        [InlineKeyboardButton(get_text(constants.BTN_CANCEL, lang_code), callback_data=f"{CB_EA_CANCEL_SINGLE}{alert_data['id']}")]
    ]
...
async def edit_alert_ask_value(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    alert_data = context.user_data['edit_alert_data']
    ticker = constants.REVERSE_ASSET_MAP.get(alert_data['asset_id'], alert_data['asset_id'])
    text = get_text(constants.PROMPT_EDIT_ALERT_NEW_VALUE, lang_code, ticker=ticker, current_value=alert_data['target_value'])
    await query.edit_message_reply_markup(reply_markup=None)
...
async def edit_alert_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    text = get_text(constants.PROMPT_EDIT_ALERT_CONFIRM, lang_code, ticker=ticker, new_condition=html.escape(new_condition), new_value=new_value, alert_id=alert_id)
    keyboard = [[
        InlineKeyboardButton(get_text(constants.BTN_YES, lang_code), callback_data=f"{CB_EA_SAVE}{alert_id}"),
        InlineKeyboardButton(get_text(constants.BTN_NO, lang_code), callback_data=f"{CB_EA_CANCEL_SINGLE}{alert_id}")
    ]]
    if update.callback_query:
...
async def edit_alert_save(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    alert_id = context.user_data['edit_alert_id']
    updates = {}
    if 'new_alert_condition' in context.user_data: updates['condition'] = context.user_data['new_alert_condition']
    if 'new_alert_value' in context.user_data: updates['target_value'] = context.user_data['new_alert_value']
    ...
    if not updates:
        ...
        final_text = get_text(constants.MSG_EDIT_ALERT_SUCCESS, lang_code, alert_id=alert_id) if success else get_text(constants.MSG_ERROR_DB, lang_code)
...
async def edit_alert_cancel(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if query and query.message:
        ...
        await update.message.reply_text(cancel_message, reply_markup=common_handlers.get_main_keyboard(lang_code))
    return ConversationHandler.END

# --- Fallback Handlers ---
DIALOG_NAME_ALERT = "добавления/редактирования алерта"
common_fallbacks_for_alert_dialogs = [
    CommandHandler(c, lambda u, c: common_handlers.cancel_conversation_and_call_command(u, c, h, DIALOG_NAME_ALERT))
    for c, h_name in [
    ...
    ] for h in [getattr(globals().get(h_name.split('_')[0] + '_handler', common_handlers), h_name, None)] if h
]
...
add_alert_conv_handler = ConversationHandler(
    ...
    states={
        ...
        CHOOSE_TYPE: [CallbackQueryHandler(received_alert_type, pattern=f"^{constants.CB_ACTION_CHOOSE_ALERT_TYPE}")],
        ASK_PRICE_CONDITION: [
            CallbackQueryHandler(received_price_condition_button, pattern=f"^{constants.CB_ALERT_COND_GT}$"),
            CallbackQueryHandler(received_price_condition_button, pattern=f"^{constants.CB_ALERT_COND_LT}$"),
        ],
...
edit_alert_conv_handler = ConversationHandler(
    entry_points=[CallbackQueryHandler(edit_alert_start, pattern=f"^{CB_EA_START}{constants.ALERT_ID_REGEX_PART}$")],
    states={
        EDIT_ALERT_CHOICE: [
            CallbackQueryHandler(edit_alert_ask_condition, pattern=f"^{CB_EA_SET_COND_PREFIX}{constants.ALERT_ID_REGEX_PART}$"),
            CallbackQueryHandler(edit_alert_ask_value, pattern=f"^{CB_EA_SET_VAL}{constants.ALERT_ID_REGEX_PART}$")
        ],
        EDIT_ALERT_CONDITION: [CallbackQueryHandler(edit_alert_receive_condition, pattern=f"^{CB_EA_SET_COND_PREFIX}{constants.ALERT_ID_REGEX_PART}_(>|<)$")],
        EDIT_ALERT_VALUE: [MessageHandler(filters.TEXT & ~filters.COMMAND, edit_alert_receive_value)],
        EDIT_ALERT_CONFIRM: [
            CallbackQueryHandler(edit_alert_save, pattern=f"^{CB_EA_SAVE}{constants.ALERT_ID_REGEX_PART}$"),
...
application_add_handler_alert_deletion_confirmation = [
    CallbackQueryHandler(delalert_confirmed_callback, pattern=f"^{constants.CB_ACTION_DEL_ALERT_CONFIRMED}{constants.ALERT_ID_REGEX_PART}$"),
    CallbackQueryHandler(delalert_cancelled_callback, pattern=f"^{constants.CB_ACTION_DEL_ALERT_CANCELLED}{constants.ALERT_ID_REGEX_PART}$")
]
