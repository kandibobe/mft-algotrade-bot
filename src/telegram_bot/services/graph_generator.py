...
def generate_candlestick_graph(
    ...
    if not historical_data or len(historical_data) < MIN_POINTS_FOR_GRAPH_RENDER:
        logger.warning(
            f"Недостаточно данных для генерации графика {asset_symbol} ({len(historical_data)} < {MIN_POINTS_FOR_GRAPH_RENDER})."
        )
        return None

    logger.debug(
        f"Генерация свечного графика для {asset_symbol} ({period_days} дн., {len(historical_data)} точек)"
    )
    fig = None
    ...
    try:
        ...
        if df.isnull().values.any():
            logger.warning(
                f"DataFrame для графика {asset_symbol} содержит NaN значения после обработки. Попытка удалить строки с NaN."
            )
            df.dropna(inplace=True)
            if len(df) < MIN_POINTS_FOR_GRAPH_RENDER:
                logger.warning(
                    f"После удаления NaN осталось недостаточно данных для графика {asset_symbol} ({len(df)} < {MIN_POINTS_FOR_GRAPH_RENDER})."
                )
                return None
        ...
        if fig:
            try:
                plt.close(fig)
                logger.debug(f"Фигура графика '{asset_symbol}' закрыта.")
            except Exception as close_err:
                logger.error(f"Ошибка при закрытии фигуры графика '{asset_symbol}': {close_err}")


def generate_trend_graph(
    data: list[tuple[DateType, float] | float], title: str, y_label: str
) -> bytes | None:
    """Генерирует линейный график с линией тренда."""
    if not data or len(data) < 2:
        logger.warning(
            f"Недостаточно данных для генерации графика тренда для '{title}' ({len(data)} < 2)."
        )
        return None
    ...
    try:
        ...
        if fig:
            try:
                plt.close(fig)
                logger.debug(f"Фигура графика '{title}' закрыта.")
            except Exception as close_err:
                logger.error(f"Ошибка при закрытии фигуры графика '{title}': {close_err}")
