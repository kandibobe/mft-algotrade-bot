...
class TradingMetricsExporter:
    ...
    def _init_metrics(self) -> None:
        ...
        self.circuit_breaker_status = Gauge(
            f'{self.namespace}_circuit_breaker_status',
            'Circuit breaker status (0=off, 1=on)'
        )
        
        # System metrics
        self.metrics_up = Gauge(
            f'{self.namespace}_up',
            'Metrics exporter status (1=up, 0=down)'
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Stoic Citadel Prometheus Metrics Exporter"
    )
    parser.add_argument(
        "--host",
