...
class RiskManager:
    ...
    def save_state(self) -> None:
        ...
        try:
            ...
            with sqlite3.connect(self._db_path) as conn:
                conn.execute("CREATE TABLE IF NOT EXISTS risk_state (id INTEGER PRIMARY KEY, data BLOB, timestamp DATETIME)")
                ...
                conn.execute("INSERT OR REPLACE INTO risk_state (id, data, timestamp) VALUES (1, ?, ?)",
        ...
        try:
            ...
            with sqlite3.connect(self._db_path) as conn:
                ...
                if row:
                    ...
                    self._exchange_balances = {k: Decimal(v) for k, v in state.get("exchange_balances", {}).items()}
    ...
    def check_market_kill_switch(self) -> tuple[bool, str | None]:
        ...
        try:
            ...
            last_change = (btc_data['close'].iloc[-1] - btc_data['close'].iloc[-2]) / btc_data['close'].iloc[-2]
            if last_change < -0.05:
    ...
    def evaluate_trade(
        ...
        **kwargs
        ...
        correlation_pairs = getattr(u_cfg, 'correlation_pairs_block', ["BTC", "ETH"]) if u_cfg else ["BTC", "ETH"]
        correlation_threshold = getattr(u_cfg, 'correlation_threshold_block', 0.8) if u_cfg else 0.8
        target_pair_base = symbol.split("/")[0] if "/" in symbol else symbol.split(":")[0] if ":" in symbol else symbol
        ...
        if target_pair_base in correlation_pairs:
            for other_pair_base in correlation_pairs:
                ...
                for open_symbol, pos in self._positions.items():
                    open_base = open_symbol.split("/")[0] if "/" in open_symbol else open_symbol.split(":")[0] if ":" in open_symbol else open_symbol
                    if open_base == other_pair_base:
                        ...
                        if open_side == side:
                            ...
                            try:
                                ...
                                logger.error(f"Correlation calculation failed: {e}. Blocking for safety.")
        ...
        if len(self._positions) >= 2:
            try:
                ...
                if avg_corr > 0.85:
                    logger.warning(f"High portfolio correlation detected ({avg_corr}), dampening risk.")
