...
async def premium_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    premium_status_key = constants.MSG_PREMIUM_STATUS_ACTIVE if is_premium else constants.MSG_PREMIUM_STATUS_INACTIVE
    premium_status_text = get_text(premium_status_key, lang_code)

    premium_info_text = get_text(
        constants.MSG_PREMIUM_INFO, lang_code,
    ...
    if effective_message:
        ...
        if query:
             try:
                  await query.edit_message_text(premium_info_text, parse_mode=ParseMode.HTML, disable_web_page_preview=True)
                  edit_success = True
             except Exception as e: logger.warning(f"Не удалось отредактировать сообщение Premium Info: {e}")
...
async def market_data_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        [
            ...
            InlineKeyboardButton(f"{get_text('menu_cmd_gas', lang_code)}", callback_data="command:/gas")
        ],
        [
            InlineKeyboardButton(get_text('btn_volatility', lang_code), callback_data="command:/volatility"),
            InlineKeyboardButton(f"{get_text('menu_cmd_trending', lang_code)}", callback_data="command:/trending")
        ],
        [
            InlineKeyboardButton(f"{get_text('menu_cmd_funding', lang_code)}", callback_data="command:/funding"),
            InlineKeyboardButton(f"{get_text('menu_cmd_tvl', lang_code)}", callback_data="command:/tvl")
        ],
        [
            InlineKeyboardButton(f"{get_text('menu_cmd_marketcap', lang_code)}", callback_data="command:/marketcap"),
            InlineKeyboardButton(f"{get_text('menu_cmd_events', lang_code)}", callback_data="command:/events")
        ]
    ]
