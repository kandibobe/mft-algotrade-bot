...
def create_bot_application() -> Application:
    ...
    application = (
        ApplicationBuilder().token(TELEGRAM_BOT_TOKEN).defaults(defaults)
        .post_init(post_init).post_shutdown(post_shutdown)
        .connect_timeout(30).read_timeout(30).get_updates_read_timeout(60)
        .build()
    )
    ...
    application.add_handler(CommandHandler(constants.CMD_REPORT, report_handler.report_command_handler))
    application.add_handler(CommandHandler(constants.CMD_SIGNAL, signal_handler.signal_command_handler))
    application.add_handler(CommandHandler(constants.CMD_VOLATILITY, volatility_handler.volatility_command_handler))
