...
def set_language(user_id: int, lang_code: str) -> bool:
    if lang_code not in SUPPORTED_LANGUAGES:
        logger.warning(
            f"Неподдерживаемый язык '{lang_code}' для user {user_id}. Используется '{DEFAULT_LANGUAGE}'."
        )
    ...
    if success:
        try:
            ...
            logger.warning(
                f"Не удалось обновить кеш языка для user {user_id} после смены на '{lang_code}': {cache_err}"
            )
    else:
...
def add_asset_to_watchlist(user_id: int, ticker: str) -> str:
    ...
    if current_count >= limits["watchlist"]:
        logger.warning(
            f"User {user_id} достиг лимита watchlist ({current_count}/{limits['watchlist']})."
        )
        return OPERATION_FAILED_LIMIT
    asset_info = constants.SUPPORTED_ASSETS.get(ticker_upper)
    if not asset_info:
        logger.warning(
            f"Попытка добавить невалидный тикер '{ticker_upper}' в watchlist user {user_id}."
        )
    ...
    if asset_type not in [constants.ASSET_CRYPTO, constants.ASSET_FOREX]:
        logger.warning(
            f"Попытка добавить неподдерживаемый тип актива '{asset_type}' ({ticker_upper}) в watchlist user {user_id}."
        )
        return OPERATION_FAILED_INVALID
    ...
    if asset_info:
        ...
        if ticker_or_id in constants.REVERSE_ASSET_MAP:
            ...
            if not found_by_direct_id_check:
                logger.warning(
                    f"Попытка удалить неподдерживаемый тикер/ID '{ticker_or_id}' из watchlist user {user_id}."
                )
                return OPERATION_FAILED_NOT_FOUND

    if not asset_id_to_delete:
        logger.error(
            f"Не удалось определить asset_id для удаления '{ticker_or_id}' user {user_id}."
        )
        return OPERATION_FAILED_INVALID
...
def add_user_alert(
    ...
    if current_count >= limits["alerts"]:
        logger.warning(
            f"User {user_id} достиг лимита алертов ({current_count}/{limits['alerts']})."
        )
    ...
    if alert_type == constants.ALERT_TYPE_PRICE:
        ...
        if (
            condition not in [">", "<"]
            or target_value not in [30, 70]
            or asset_type != constants.ASSET_CRYPTO
        ):
            return OPERATION_FAILED_INVALID, None
    else:
        return OPERATION_FAILED_INVALID, None
    ...
    return (
        (OPERATION_SUCCESS, alert_id) if alert_id is not None else (OPERATION_FAILED_DB_ERROR, None)
    )
...
def delete_user_price_alert(user_id: int, alert_id: int) -> str:
    ...
    return (
        OPERATION_SUCCESS
        if db_manager.delete_price_alert(user_id, alert_id)
        else OPERATION_FAILED_NOT_FOUND
    )
...
def update_user_price_alert_fields(user_id: int, alert_id: int, updates: dict[str, Any]) -> bool:
    ...
    allowed_updates = {
        ...
        if (k == "condition" and v in [">", "<"])
        or (k == "target_value" and isinstance(v, (int, float)) and v > 0)
    }
    return (
        db_manager.update_price_alert_fields(user_id, alert_id, allowed_updates)
        if allowed_updates
        else False
    )
