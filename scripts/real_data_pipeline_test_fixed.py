"""
Fixed pipeline test with proper handling of infinite values and data issues.
"""
...
def clean_dataframe(df):
    """Clean dataframe by removing infinite values and handling NaNs."""
    # Make a copy
    df_clean = df.copy()
    ...
    if nan_count > 0:
        print(f"   Warning: {nan_count} NaN values remain after cleaning")
...
def test_with_improved_configs(df):
    ...
    # Clean features
    df_features = clean_dataframe(df_features)
    print(f"   After cleaning: {df_features.shape[1]} features")
    ...
    # Clean X before feature selection
    X = clean_dataframe(X)
    ...
    for model_name, model in models.items():
        ...
        if folds:
            ...
            if results['accuracy']:  # Check if we have any successful folds
    ...
    if all_results:
        ...
        return {
            ...
            'feature_importance': feature_selector.get_feature_importance() if hasattr(feature_selector, 'get_feature_importance') else None
        }
    else:
...
def run_complete_pipeline_analysis():
    ...
    print("="*80)
    print("COMPLETE REAL DATA PIPELINE ANALYSIS (FIXED VERSION)")
    print("="*80)
