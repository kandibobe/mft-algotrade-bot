"""
Stoic Citadel - Strategy V1 (Enhanced Production Grade)
=======================================================
...
5. Dynamic Position Sizing: Based on volatility (ATR)
6. Emergency Exits: Timeout and strong profit protection
...
----------
v1.1.0 (2024-12):
  - Enhanced regime detection with multiple timeframes
  - Improved entry/exit confirmation logic
  - Better risk management with dynamic sizing
  - Added advanced logging for debugging
...
class StoicStrategyV1(IStrategy):
    ...
    def populate_indicators_btc(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        logger.info(f"BTC Regime: {'BULL' if dataframe['regime_bull'].iloc[-1] else 'BEAR'}")
    ...
    def custom_stake_amount(
        ...
        **kwargs
        ...
        if volatility_pct > 0.05:  # High volatility
            ...
            logger.debug(f"{pair}: High volatility ({volatility_pct:.2%}), reducing position to 60%")
        elif volatility_pct > 0.03:  # Medium volatility
            adjusted_stake = proposed_stake * 0.8
            logger.debug(f"{pair}: Medium volatility ({volatility_pct:.2%}), reducing position to 80%")
        else:  # Low volatility
            adjusted_stake = proposed_stake
            logger.debug(f"{pair}: Normal volatility ({volatility_pct:.2%}), full position")
    ...
    def confirm_trade_entry(
        ...
        **kwargs
        ...
        if hour in [0, 1, 2, 3, 4, 5]:
            logger.info(f"Entry rejected for {pair} - Low liquidity hours (UTC {hour}:00)")
            return False
        ...
        logger.info(f"Entry confirmed for {pair} at {rate:.8f}")
        return True
    ...
    def custom_exit(
        ...
        **kwargs
        ...
        if trade_duration_hours > 24 and current_profit < -0.02:
            logger.warning(f"Emergency exit for {pair}: Open {trade_duration_hours:.1f}h with {current_profit:.2%} profit")
            return "emergency_timeout_exit"

        # Take profit on strong moves
        if current_profit > 0.08:  # 8% profit
            logger.info(f"Strong profit exit for {pair}: {current_profit:.2%}")
