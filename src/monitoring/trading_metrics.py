...
class TradingMetrics:
    ...
    def _init_metrics(self) -> None:
        ...
        self.daily_pnl = Gauge(
            f"{self.namespace}_daily_pnl_usd", "Daily realized PnL in USD", registry=self.registry
        )

        self.daily_pnl_pct = Gauge(
            f"{self.namespace}_daily_pnl_pct", "Daily PnL as percentage", registry=self.registry
        )

        self.unrealized_pnl = Gauge(
            f"{self.namespace}_unrealized_pnl_usd", "Unrealized PnL in USD", registry=self.registry
        )

        self.total_pnl = Counter(
            f"{self.namespace}_total_pnl_usd", "Total cumulative PnL in USD", registry=self.registry
        )
        ...
        # === Win Rate Metrics ===
        self.win_rate = Gauge(
            f"{self.namespace}_win_rate", "Current win rate (0-1)", registry=self.registry
        )
        ...
        self.sharpe_ratio = Gauge(
            f"{self.namespace}_sharpe_ratio", "Rolling Sharpe ratio", registry=self.registry
        )

        self.sortino_ratio = Gauge(
            f"{self.namespace}_sortino_ratio", "Rolling Sortino ratio", registry=self.registry
        )

        self.var_95 = Gauge(
            f"{self.namespace}_var_95_pct", "95% Value at Risk", registry=self.registry
        )
        ...
        self.orders_filled = Counter(
            f"{self.namespace}_orders_filled_total", "Total orders filled", registry=self.registry
        )
    ...
    def update_account(self, balance: float, equity: float, unrealized: float) -> None:
    ...
    def record_trade(self, side: str, pnl_pct: float, duration_hours: float) -> None:
    ...
    def update_performance(
        self, win_rate: float, profit_factor: float, sharpe: float, sortino: float, var_95: float
    ) -> None:
    ...
    def update_exposure(self, total_usd: float, percentage: float) -> None:
    ...
    def record_order(
        self, order_type: str, filled: bool = True, slippage: float = 0, commission: float = 0
    ) -> None:
    ...
    def observe_latency(self, latency_seconds: float, metric_type: str = "strategy") -> None:
        """Observe latency."""
        if not self._enabled:
            return
